...
C
      ...
      CHARACTER(8) QNAME(10),QTYPE(10),PRTP(4,9),PTP(2),UNAME
...
C        NAMELIST
     ...
     2                 EMAX,  EMAXK, EUNITS,IASYMU,IBOUND,IDENT,
     3                 IPHIFL,IOSNGP,ISYM,  ISYM2, ITYPE, IVLU,
...
C
 5204 N=0
      ...
      DO 4001 I=1,NSTATE
        JI=JSTATE(I)
        LI=JTOT+LDLD-JI
        IF (LI.LT.IABS(JTOT-JI) .OR. LI.GT.(JTOT+JI) ) GOTO 4001
        N=N+1
        IF (LCOUNT) GOTO 4001
        JSINDX(N)=I
        L(N)=LI
...
C  SKIP IF J.LT.MVALUE OR EXCLUDED BY EXCHANGE SYMMETRY.
        ...
        IF (JSTATE(IOFF+I).LT.IABS(MVALUE)) GOTO 5221
        IF ((IBOUND.NE.0 .OR. JZCSFL.NE.0) .AND. JTOT.LT.IABS(MVALUE))
     1    GOTO 5221
     ...
     1      GOTO 5221
        ...
        IF (LCOUNT) GOTO 5221
        JSINDX(N)=I
        L(N)=IABS(JTOT+JZCSFL*(JSTATE(IOFF+I)-2*IABS(MVALUE)))
...
C  APR 94, ALLOW FOR NEGATIVE SIG-INDEX
      DO 8900 I=1,N
        EINT(I)=CINT * ELEVEL(IABS(JSTATE(NSTATE*(NQN-1)+JSINDX(I))))
 8900 CONTINUE
      DO 8950 I=1,N
...
C
 ...
 5000 IF (N.LE.0) N=0
      IF (N.EQ.0 .AND. IPRINT.GE.1)
     1  WRITE(6,*) ' BASIS SIZE IS 0 FOR THIS COMBINATION'//
     2             ' OF SYMMETRY LABELS'
      RETURN
C
C * * * * * * * * * * * * * * * * * * * * * * * * * * * END OF BASE * *
C
...
C  BELOW APPLIES EFACT TO ROTI,ELEVEL
      IF (EFACT.NE.1.D0 .AND. ITYPE.EQ.9) THEN
        WRITE(6,*)' *** WARNING - ALL ELEMENTS OF ROTI ARRAY',
     1            ' MULTIPLIED BY ',EFACT
      ENDIF
      DO 6901 I=1,MXROTS
 6901   ROTI(I)=ROTI(I)*EFACT
      DO I=1,MXELVL
        ELEVEL(I)=ELEVEL(I)*EFACT
      ENDDO
...
C
 1004 NQN = 8
      ...
      QNAME(7) = QTYPE(9)
      MXPAR = 2
      CALL SET4(NSTATE,JSTATE,ATAU,EFACT,IASYMU,IPRINT)
      IF (ITYPE.LE.10) GOTO 2000
...
C
 1006 IGO=9006
      ...
      NQN=6
      MXPAR=2
      CALL SET6(LEVIN,EIN,NSTATE,JSTATE,ATAU,EFACT,IASYMU,IPRINT)
      IF (ITYPE.LE.10) GOTO 2000
...
C  BUILD FORMAT STATEMENTS
     ...
     1                 ' QUANTUM LABEL'//PLUR(MIN(NQN-1,2))

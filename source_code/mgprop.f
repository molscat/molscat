...
C  (LOG-DERIVATIVE MATRIX AT REND) AND NODES.
...
C  INITIALIZE SCALARS
      H = (REND-RBEGIN)/DBLE(NSTEP) ! DR IN PAPER
      NODES = 0
...
C  SECTOR, AND W CAN BE REUSED.
            IF (K.NE.1 .OR. A(MSYMP).NE.0.0D0) THEN
               IF (IREAD) THEN
                  READ(ISCRU) U
                  DO I = 1,N
                     U(I,I)=U(I,I)-ESHIFT
...
C  NOTE THAT IN BOUND, RICHARDSON EXTRAPOLATION WILL NOT WORK WELL
C  FOR MA5 SINCE IT ASSUMES THE WRONG STEPSIZE DEPENDENCE.

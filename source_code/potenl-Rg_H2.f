...
C  Distributed under the GNU General Public License, version 3
      USE angles
      USE potential, ONLY: LAMBDA, MXLMDA, RMUNIT, EPUNIT
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      SAVE
C
C  ROUTINE TO HANDLE THE INTERMOLECULAR POTENTIAL
C
C  THIS VERSION IS FOR AN ATOM + VIBRATING DIATOM SYSTEM (ITYP=10*N+7
C  WHERE ONLY THE V=0 AND V=1 DIATOM VIBRATIONAL STATES ARE CONSIDERE
...
C     DIMENSION PNAMES(8),LOCN(8),INDX(8)
...
C     DATA PNAMES/'NLEG','NSTR','IHET','JMAXV0','JMAXV1','NGP',
C    1            'SCAL','SCALMX'/
C     DATA INDX/8*0/
      P2(X) = 1.5D0*X*X-ZH
C
C  IC IS A CONTROL PARAMETER WITH THE FOLLOWING MEANINGS:
...
C  NSTR   THE TOTAL NUMBER OF TERMS, IN THE DIATOM STRETCH EXPANSION
C         OF EACH LEGENDRE COMPONENT, WHICH ARE INCLUDED IN THE
C         VIBRATIONAL AVERAGING OVER V AND V'.
...
C  NGP     NO. OF GAUSSIAN QUADRATURE POINTS USED FOR TRANSFORMATION
C  SCAL    IF SCAL > 1.0, MATRIX ELEPHANTS OFF-DIAGONAL IN V WILL
C          BE INCREASE BY A FACTOR OF SCAL. THIS IS USEFUL WHEN
C          SEARCHING FOR VER NARROW RESONANCES.
C  SCALMX  LARGEST PERMITTED VALUE OF OFF-DIAGONAL MATRIX ELEPHANTS
...
C     LOCN(7)=LOC(SCAL)
C     LOCN(8)=LOC(SCALMX)
C     CALL NAMLIS('&POTL ',PNAMES,LOCN,INDX,8,IEOF)
C-----------------------------------------------------------------------
      READ(5,POTL)
C-----------------------------------------------------------------------
...
C  NPOTL   EQUAL TO KMAX+1, WHERE KMAX IS THE LARGEST LEGENDRE
C          POLYNOMIAL INDEX APPEARING IN THE LMB ARRAY.
C          NOTE THAT THIS IS JUST HOW NPOTL IS USED HERE, NOT
C          A GENERAL STATEMENT OF ITS USE FOR THE MOLSCAT PROGRAM.
C  RR      GIVES RM, THE UNIT OF LENGTH (IN ANGSTROMS)
C  P(1)    GIVES EPSIL, THE UNIT OF ENERGY (IN WAVENUMBERS)
C
C  SET RM,EPSIL AND NPOTL
C
      NPOTL = (2-IHET)*NLEG+IHET-1
      RR = 1.D0
      P(1) = 1.D0
...
C***********************************************************************
C**                                                                  ***
C**             RJL MAGIC POTENTIAL INITIALIZATION CODE              ***
C**                                                                  ***
C***********************************************************************
...
C** IDAMP=1 FOR THE BC DAMPING FUNCTION (CARLEY/LEROY)
C   IDAMP=2 FOR THE TT DAMPING FUNCTION (LEROY/HUTSON 1985)
      READ (5,*) NOFPOT,IDAMP
...
C     STRENGTH COEFFICIENTS FOR POTENTIAL MATRIX ELEMENTS
      ...
      IFAC = 2-IHET
      J = 1
C     ON VAX, OPEN DATA FILE SHARED SO THAT OTHER PROGRAMS CAN
C     USE IT SIMULTANEOUSLY.
C     OPEN(1,STATUS='OLD',SHARED,READONLY)
...
C** CHECK FOR EXCLUSION OF THIS (V,J) AND SET UP LMB
        ...
        IF (5*J*NLEG.LE.MXLMB) GOTO 240

        WRITE (6,230) MXLMB
  230   FORMAT ('  ****** ERROR IN POTENL - ARRAY LMB(',I7,
     1          ') IS NOT BIG ENOUGH')
        STOP
...
C** PREPARE GAUSSIAN WEIGHTS AND POINTS FOR TRANSFORMATION QUADRATURE
...
C   POLYNOMIALS AND GAUSSIAN WEIGHTS.
  ...
  400 CONTINUE
C     CALL D01BBF (D01BAZ,-Z1,Z1,0,NGP,WG,XG,IFAIL)
C     THIS VERSION USES GAUSSP IN PLACE OF NAG FOR G-LEG QUAD POINTS
      CALL GAUSSP(-1.D0,1.D0,NPT,XG,WG)
...
C  THE VALUES THEY WERE ASSIGNED ON INITIALIZATION (SEE ABOVE).
...
C*********************************************************************
C***                                                               ***
C***               RJL MAGIC POTENTIAL ROUTINE                     ***
C***                                                               ***
C*********************************************************************
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C  ENTRY HERE TO PREPARE ACTUAL POTENTIAL ARRAYS FOR AN UNSHIFTED
C  HOMONUCLEAR DIATOM.
      IND = 0
      R6=RR**(-6)
      R8=R6/(RR*RR)
      EXPBR=EXP(-BETA(1)*RR)
  ...
  550 CONTINUE
      RETURN
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C  HERE PREPARE POTENTIAL ARRAYS FOR HETERONUCLEAR DIATOM BY SHIFTING
C  CENTRE OF MASS BY  (DEL(J),J=1,NOFJ)  ON HOMONUCLEAR POTENTIAL. ++
  ...
  610 CONTINUE
      RETURN
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C  HERE PREPARE POTENTIAL ARRAY FOR HETERONUCLEAR DIATOM CASE WHILE
C  PERFORMING CENTRE OF MASS SHIFT PRIOR TO (DIATOM) VIBRATIONAL
C  AVERAGING.
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C  VLAM(LMB) (1/CM)  AT DISTANCE  RR (ANGSTROMS)  FROM  BC3(6,8)
C  FORM FOR EACH LAMBDA.
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      SAVE
      DIMENSION AA(2,5),C8(2,5),C6(2,5),BETA(2),VLAM(2),D(4)
...
C  DIATOM STRETCH OF  XID , FROM THE CARLEY-LEROY BC3(6,8) SURFACE.
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      SAVE

...
C  MODIFIED 7 JAN 2013 TO STORE ERED AND RMLMDA AS PART OF
...
C  ELEVEL, ENERGY.  THESE HAVE THEIR NORMAL MOLSCAT DEFINITIONS.
      ...
      END
C------------------------------------------------------ END OF LDRWGV
      INTEGER FUNCTION LDRWPH(ILDSVU, LWRITE, JTOT, INRG, EN, IEXCH,
     1                        WT, M, NCH, FIELD1, FIELD2, ERED, RMLMDA)
CER   INTEGER FUNCTION LDRWPH(ILDSVU, LWRITE, JTOT, INRG,
CER  *     EN, IEXCH, WT, M, NCH, FIELD1, FIELD2)
C
C  READ/WRITE PROPAGATION HEADER.
C  READ/WRITE SCALAR HEADER FOR A SINGLE PROPAGATION.  JTOT AND M HAVE
...
C  ERED AND RMLMDA (USUAL MOLSCAT DEFINITIONS) ADDED TO CALL LIST
...
C  MARKED C16 IN PLACE OF THE CURRENT VERSIONS
      ...
      PARAMETER (MAXEXT=10)
      LOGICAL LWRITE
      INTEGER ILDSVU, JTOT, INRG,IEXCH, M, NCH
      DOUBLE PRECISION URED, EN, WT,FIELD1, FIELD2, ERED, RMLMDA

C  INCLUDE COMMON BLOCK FOR EXTERNAL FIELDS
      COMMON/EXTNL/NXTVAR,IVARXT,EXTVAR(0:MAXEXT),IXTVAR(0:MAXEXT),
     1             EXTNAM(0:MAXEXT),EXTUNT(0:MAXEXT)
      DOUBLE PRECISION EXTVAR
      INTEGER NXTVAR,IXTVAR,IVARXT
      CHARACTER*20 EXTNAM
      CHARACTER*6 EXTUNT
      ...
      IF (LWRITE) THEN
         WRITE(ILDSVU,IOSTAT=IERR) JTOT, INRG, EN, IEXCH, WT, M, NCH,
     1                             ERED, RMLMDA, IVARXT,NXTVAR
         WRITE(ILDSVU,IOSTAT=IERR) (EXTVAR(IVAL),IVAL=MIN(IVARXT,1),
     1                              NXTVAR)
C16      WRITE(ILDSVU,IOSTAT=IERR) JTOT, INRG, EN, IEXCH, WT, M, NCH,
C16  1                             FIELD1, FIELD2, ERED, RMLMDA
CER      WRITE(ILDSVU,IOSTAT=IERR) JTOT, INRG, EN,
CER  *                             IEXCH, WT, M, NCH, FIELD1, FIELD2
      ELSE
         IF (ILDSVU.GT.0) THEN
            READ(ILDSVU,IOSTAT=IERR) JTOT, INRG, EN, IEXCH, WT, M, NCH,
     1                               ERED, RMLMDA, IVARXT,NXTVAR
            READ(ILDSVU,IOSTAT=IERR) (EXTVAR(IVAL),IVAL=MIN(IVARXT,1),
     1                                NXTVAR)
C16         READ(ILDSVU,IOSTAT=IERR) JTOT, INRG, EN, IEXCH, WT, M, NCH,
C16  1                               FIELD1, FIELD2, ERED, RMLMDA

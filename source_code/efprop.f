      SUBROUTINE EFPROP(N, RBEGIN, REND, NSTEPS, PSIB, RAB, PSIA,
...
C  R IS WRITTEN TO PROPAGATION FILE BY DWPROP, AND READ IN HERE, AND IS
C  ALSO WRITTEN OUT TO WAVEFUNCTION FILE.
C
C  ON ENTRY: RBEGIN}
C            REND  } ARE LIMITS OF RADIAL PROPAGATION VARIABLE,
C            N       IS THE SIZE OF THE BASIS,
C            NSTEPS  IS THE NUMBER OF STEPS USED IN THE RADIAL PROPAGATION
...
C     R=RBEGIN
      R=REND
C
      DO 100 ISTEP=1,NSTEPS+1
C  SET UP PARAMETERS FOR DOING SIMPSONS RULE INTEGRATION
        IF (ISTEP.EQ.1 .OR. ISTEP.EQ.NSTEPS+1) THEN
          A=1D0

      SUBROUTINE IOSBIN(NVC,ITYPX,MX,IASYMU,IPHIFX,IOSNG,IPRINT)
...
C  Distributed under the GNU General Public License, version 3
      ...
      USE basis_data, ONLY: ELEVEL, IDENT, J2MAX, J2MIN, J2STEP,
     b                      JLEVEL, JMAX, JMIN, JSTEP, NLEVEL, ROTI
...
C  SPECIFICATIONS FOR ENTRY IOSB2 . . .
      DIMENSION CENT(NVC),EINT(NVC),WVEC(NVC),VL(*),IVIX(*)
...
C  CURRENT CODE DOES NOT ALLOW DUPLICATE VIB LEVELS.
 ...
 2102 IF (I.GT.NLEVEL) GOTO 2110
      ...
      DO II=1,NVC
        IF (LEVV(II).NE.JLEVEL(2*I)) CYCLE
...
C  PULL DOWN LIST/ DECREASE NLEVEL/ GO BACK FOR NEW I-TH LEVEL
 2120   DO J=I+1,NLEVEL
          ELEVEL(J-1)=ELEVEL(J)
          JLEVEL(2*J-3)=JLEVEL(2*J-1)
          JLEVEL(2*J-2)=JLEVEL(2*J)
        ENDDO
        NLEVEL=NLEVEL-1
        GOTO 2102
 ...
 2200 WRITE(6,603) WE
  603 FORMAT(/'  VIBRATIONAL ENERGIES (WITH V = 0 AT ZERO ENERGY)',
     1       ' CALCULATED FROM WE =',F10.4)
      IF (WEXE.NE.0.D0) WRITE(6,604) WEXE
  604 FORMAT(67X,'CORRECTED FOR WEXE =',F10.6)
      DO I=1,NVC
        FV=LEVV(I)
        EV(I)=WE*FV-WEXE*FV*(FV+1.D0)
...
C
 6000 ILOFF=2
      ...
      EV(1)=0.D0
      LEVV(1)=0
      CALL SET6I(JLEVEL,MXJL,NLEVEL,MXA,IASYMU,IPRINT)
      MX=MXA
C ***
...
C ***
      ...
      DO I=1,NLEVEL
        MAXNUM=MAX(MAXNUM,JLEVEL(4*I-3))
      ENDDO
      MXK=MAXNUM
      GOTO 9000
C
...
C  DETERMINE NO. OF LAMBDA, MU SYMMETRIES (MXXXXL)
      ...
      DO L=IZ,LMBDMX
        MTOP=MIN(MUMX,L)
      DO M=IZ,MTOP,IC0
        IF (IH0.EQ.2 .AND. LODD(L,M)) CYCLE
...
C  AND COUNT NQL (NUMBER OF QLT VALUES)
 3524 LMAX=0
      ...
      DO M=IZ,MTOP,IC0
        ...
        LMAX=LMAX+1
        DO I=IZ,M,IC0
          IF (IH0.EQ.2 .AND. LODD(L,I)) CYCLE
...
C  N.B. WE KEEP EVEN AND ODD L FOR HOMONUCLEARS, BUT SET TO 0. IF NEC
          PWGHT(NX,L)=0.D0
          IF (IH0.EQ.2 .AND. L-2*(L/2).EQ.0) CYCLE
...
C  IL COUNTS SYMMETRIES IN POTENTIAL TO L1MAX,L2MAX
      ...
      DO LL=LLO,LHI
        ...
        DO IX3=1,NGM
          I=I+1
          PWGHT(I,IL)=GWT(IX1)*GWT(IST2+IX2)*GWT(IST3+IX3)*
...
!  Start of long DO loop #2
 ...
 4317     L2TOP=L2MAX
        ...
        DO L2=IZ,L2TOP,IC0
          ...
          PTL2=PARSGN(L2)
          DO IX=1,NGP2
            PL2(IX)=PLM(L2,M,COSA(IST2+IX))*PTL2
          ENDDO
 4318     LLO=ABS(L1-L2)
          LHI=L1+L2
        DO LL=LLO,LHI
          ...
          DO IX3=1,NGM
            I=I+1
            VLI(I,IL)=VLI(I,IL)+PL1(IX1)*PL2(IX2)*COSM(IX3)*TJ
          ENDDO
          ENDDO
          ENDDO
        ENDDO
...
C  STORE IXQL, LM ONLY FOR M=0 PASS ONLY.
          ...
          LM(2,IL)=L2
          LM(3,IL)=LL
 4328     IF (L1.LT.M .OR. L2.LT.M) CYCLE
          XL=LL
          ...
          DO IX3=1,NGM
            I=I+1
            PWGHT(I,IL)=PWGHT(I,IL)+PL1(IX1)*PL2(IX2)*COSM(IX3)*TJ
          ENDDO
          ENDDO
          ENDDO
        ENDDO
...
C  SETUP PWGHT
      ...
      DO M=IZ,MTOP,IC0
        ...
        DO NX=1,NGM
          IV=IV+1
          PWGHT(IV,IL)=GWT(IX)*GWT(NGL+NX)*PLM(L,M,COSA(IX))*
  ...
  649 FORMAT(/'  BELOW ARE INDICES TO SYMMETRIES IN QLT'/
     &       /'   IN QLT  LM1  L  M   LM2  L  M   CODE')
      DO I=1,NQL
        IL=IXQL(1,I)
        IX=IXQL(2,I)
        WRITE(6,648) I,IL,LM(1,IL),LM(2,IL),IX,LM(1,IX),LM(2,IX),
     1               IXQL(3,I)
...
C  N.B. WE SHOULD HAVE LLL=0 AND IL=1    *** DEBUGGING ONLY ***
  ...
  672       FORMAT(/'  IOSB2 (FEB 92) -- ERROR.  LLL.NE.0 .OR IL.NE.1',
     ...
     1                  (NV.EQ.IVP .AND. NVP.EQ.IV))) CYCLE
...
C  RETURN INDX=-1 IF L, M1, OR M2 ARE SYMMETRY FORBIDDEN.
      INDX=-1
      IF (M1-IC0*(M1/IC0).NE.0) RETURN
      IF (M2-IC0*(M2/IC0).NE.0) RETURN
      IF (IH0.EQ.2 .AND. LODD(LX,M1)) RETURN
      IF (IH0.EQ.2 .AND. LODD(LX,M2)) RETURN

      ICODE=ICDE
      IF (M1.EQ.M2) ICODE=0
      INDX=0
      DO I=1,NQL
        IF (LX.NE.LM(1,IXQL(1,I))) CYCLE
        IF (M1.NE.LM(2,IXQL(1,I))) CYCLE
        IF (M2.NE.LM(2,IXQL(2,I))) CYCLE
        IF (ICODE.NE.IXQL(3,I)) CYCLE
        INDX=I
        RETURN

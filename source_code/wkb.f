      SUBROUTINE WKB(N,MXLAM,NHAM,SREAL,SIMAG,VL,IV,EINT,CENT,
     1               DWVEC,L,
...
C  VARIABLES FOR MOLSCAT COMPATIBILITY . . .
      DIMENSION W(1),SIMAG(*),SREAL(*),L(*),EINT(*),CENT(*),
     1          DIAG(1),VL(*),IV(*)
      ALLOCATABLE :: P(:)
C
C
...
C  IF POTENTIAL IS NOT DECREASING, TRY BACKING UP . . .
 ...
 1000 CALL HAMMAT(W,N,R,P,VL,IV,ERED,EINT,CENT,EP2RU,CM2RU,
...
C  STEP IN DIRECTION OF OPPOSITE SIGN FOR POTENTIAL.
 ...
 1210 SLOPE=(YR-YL)/(XR-XL)
      XINT=YL-SLOPE*XL
      XNEW=-XINT/SLOPE
      CALL HAMMAT(W,N,XNEW,P,VL,IV,ERED,EINT,CENT,EP2RU,CM2RU,
     1            RSCALE,DIAG,MXLAM,NHAM,IPRINT)
      YNEW=W(1)
      IT=IT+1
...
!  Start of long DO loop #1
      DO NPOINT=NSTART,NHI,NADD
        ...
        DO J=1,NPOINT
          ...
          CALL HAMMAT(W,N,R,P,VL,IV,ERED,EINT,CENT,EP2RU,CM2RU,
     1                RSCALE,DIAG,MXLAM,NHAM,IPRINT)
C  HAMMAT GIVES NEGATIVE OF WHAT WE WANT
          W(1)=-W(1)
...
C  JUDGE AS ROUND-OFF ERROR IF ABS(W) LE. 0.001*ERED
 ...
 2109     DW=W(1)
          ...
          TOTAL=TOTAL+WT*F
        ENDDO
        ETA=XKR*TOTAL+(SQRT(DCENT)-XKR)*PI*0.5D0
...
C  SET CONVERGENCE FLAG, IF CONVERGENCE IS REALLY POOR
 ...
 2900 IF (IPRINT.GE.4) WRITE(6,612) NPSV,X2,DR0,NPI,ETA
  612 FORMAT(/'  * * * NOTE.  WKB PHASE SHIFT BY',I4,'-POINT QUAD,'
     ...
     2       F9.5)
C
C  CONVERT PHASE SHIFT TO SREAL, SIMAG / RESTORE FOR RETURN
      DEALLOCATE (P)

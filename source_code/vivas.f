      SUBROUTINE VIVAS(N,NSQ,MXLAM,NPOTL,
     1                 RMAT,VECNEW,W,VL,IVL,EINT,CENT,P,
     3                 A1,A1P,B1,B1P,WKS,
     4                 G1,G1P,G2,G2P,COSX,SINX,
     5                 SINE,DIAG,XK,XSQ,TSTORE,W0,
     6                 W1,W2,EYE11,EYE12,EYE22,VECOLD,
     7                 RMIN,RMAX,DRNOW,DRMAX,TLDIAG,TOFF,ERED,ESHIFT,
     8                 RMLMDA,IPRINT,IREAD,IWRITE,ISC)
...
C  PRINT CONTROL DATA
  ...
  150 RNOW = RMIN+DRNOW
      IF (IWRITE) WRITE(ISC) RMIN,RMAX,DRINT,DIAGI,RMID,IALPHA,BALPHA,
     X                       ALPHA1,ALPHA2,IALPHA
      IF (.NOT. IREAD) GOTO 160
      READ(ISC) RMIN,RMAX,DRINT,DIAGI,RMID,IALPHA,BALPHA,
     X          ALPHA1,ALPHA2,IALPHA
C-------------------------------------------------------------------
...
C  CALCULATIONS.
C-------------------------------------------------------------------
  350 IF (IWRITE) WRITE(ISC) ISTEP,RNOW,DRNOW,LAST,N,DIAG,TSTORE,W0,W1,
     X                       W2,VECNEW,NTRVL,RMIDI,RLAST,RCENT,ITRANS
...
C-------------------------------------------------------------------
  ...
  630 XBAR = 0.D0
     ...
     1           DIAG(1),DIAG(N),RNOW,RMAX)
      ...
      IF (ABS(RMAX-RNOW-DRINT).LT.ABS(0.01D0*DRINT)) DRINT = RMAX-RNOW
      IF ((RMAX-RNOW-DRINT)*DRINT.LT.0.D0) DRINT = RMAX-RNOW
      RMID = RNOW
  ...
  650 IF (ABS(RMAX-RNOW-DRNOW).LT.ABS(0.01D0*DRNOW)) DRNOW = RMAX-RNOW
...
C-------------------------------------------------------------------
  ...
  690   NCOL = NCOL+N
      ...
      IF (IPRINT.GE.3) WRITE(6,3000) RMIN,RMAX,ISTEP
...
C-------------------------------------------------------------------
 ...
 1400 FORMAT(/2X,'H IV IVP IVPP ISHIFT IDIAG ISYM IPERT IALFP')
 1500 FORMAT(1X,L3,L4,L5,L7,L6,L5,4L6)
 1600 FORMAT(/1X,' ALPHA1 ALPHA2 IALPHA'/1X,2F7.2,I7)
 1700 FORMAT(/'  *** ERROR IN VIVAS.  FOR CHANNEL',I3,', REDUCED V-E =',
     1       E13.5/6X,'FOR STEP SIZE',E13.5,', COSH ARGUMENT OF',E13.5,
     2       ' WILL CAUSE OVERFLOW.'/6X,'USE A SMALLER STEP SIZE TO ',
     3       'AVOID THIS ERROR.')
 1800 FORMAT(/'  *** WARNING IN VIVAS. INTERVAL SIZE TOO LARGE, SO',
     1       ' CLOSED CHANNEL GROWTH MAY CAUSE NUMERICAL INSTABILITY.'/
     2       24X,'RNOW =',F8.3,',  DRNOW =',F8.3,', G2P(N) =',E13.5)
 1802 FORMAT(/'  *** WARNING IN VIVAS.  INTERVAL SIZE POSSIBLY TOO',
     1       ' LARGE FOR',I5,' STEPS.  INCREASE IPRINT FOR DETAILS')
 2500 FORMAT(/' RMIN      RMAX      DRNOW     DRMAX     TOFF     ',
     1       'TLDIAG')
 2600 FORMAT(F9.5,10F10.5,I10)
 2700 FORMAT(1X,I5,10E11.4,F6.3,2I5)
 2800 FORMAT(1X,9E14.7)
 2900 FORMAT(/2X,A10)
 3000 FORMAT(/'  VIVAS.   R-MATRIX PROPAGATED FROM',F12.4,'  TO',
     &       F12.4,'  IN',I6,'  STEPS.')

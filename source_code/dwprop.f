      SUBROUTINE DWPROP(N,MXLAM,NPOTL,
     1                  Y,U,VL,IV,EINT,CENT,P,
     2                  Y14,Y23,DIAG,W,W2,W3,
     3                  RBEGIN,REND,NSTEPS,ERED,ESHIFT,RMLMDA,
...
C  PROPAGATION RANGE, WITH NSTEPS STEPS BETWEEN RBEGIN AND REND.
C
...
C  CONSTRUCTION OF SCATTERING WAVEFUNCTION IS SLIGHLY DIFFERENT...
        IF (KSTEP.EQ.NSTEPS .OR. (WAVE .AND. CDRIVE.EQ.'M')) D2=D1
        IF (IREAD) THEN
          READ(ISCRU) U
          DO 360 I=1,N
...
C
        ...
        ENDIF
C  CONSTRUCTION OF SCATTERING WAVEFUNCTION IS SLIGHLY DIFFERENT...
        IF (KSTEP.NE.NSTEPS .AND. (WAVE .AND. CDRIVE.EQ.'M')) THEN
          DO J = 1, N
            DO I = J, N
              Y(I,J) = Y(I,J) + U(I,J)
            ENDDO
...
C
 ...
 910  WRITE(6,1010) IREC,IERR
      STOP
1000  FORMAT(/' ***** DWPROP: MATRIX INVERSION ERROR AT STEP K = ',I6,
     &       '.  RUN HALTED.')
1010  FORMAT(/' ***** DWPROP: ERROR WRITING TO WAVEFUNCTION FILE',2I6)
      END

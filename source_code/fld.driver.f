...
C  ARRAY TO HOLD TIME AND DATE
      ...
      CHARACTER METHOD*12
C
      LOGICAL LFSCAN,LFSRCH,NOBIS,UPWARD,IREAD,IWRITE
      LOGICAL CONVGE
C
      CHARACTER(80)  LABEL
      CHARACTER(120) LABL
...
C
     ...
     2                 DRAIRY, DRLR,   DTOL,   ENERGY, EREF,
     3                 EUNITS, FIELD,  FIXFLD, FLDMAX, FLDMIN,
     4                 IBDSUM, IBFIX,  IBHI,   IFIELD, IFVARY,
     5                 IMGSEL, INTFLG, IPRINT, IPROPL, IPROPS,
     6                 IPSI,   IPSISC, IREF,   IRMSET, ISCRU,
     7                 IWAVE,  IWAVSC, JSTEP,  JTOTL,  JTOTU,
     8                 KSAVE,  LABEL,  LASTIN, MAGEL,  MONQN,
     9                 MHI,    MSET,   MXCALC, NFVARY, NNRG,
     A                 NOBIS,  NODMAX, NODMIN, NUMDER, PHILW,
     B                 PHIST,  POWRX,  PRNTLV, RMATCH, RMAX,
     C                 RMID,   RMIN,   STEPS,  SCALAM, THETLW,
     D                 THETST, TOLHI,  URED,   WKBMN,  WKBMX,
C
...
C
 ...
 1050   FORMAT('  *** WARNING. THE OPTION TO REUSE A SCRATCH FILE ',
     1         'FROM A PREVIOUS RUN IS NOT SUPPORTED IN FIELD')
        ISCRU=IABS(ISCRU)
      ENDIF
C
      IF (ISCRU.NE.0) THEN
...
C
 ...
 3027   FORMAT('  BOUND STATE POSITIONS WILL BE IDENTIFIED WITH AN ',
     1         'ACCURACY OF ',1PG12.5,' ',A)
...
C  RESET VALUES READY FOR LOOP LATER ON
 ...
 1014   FORMAT(/'  INPUT ENERGY VALUES CONVERTED FROM ',A/
     ...
     2          'ALPHANUMERIC INPUT =',A4)
        ...
        WRITE(6,2030) TRIM(UNAME),(ENERGY(I),I=1,NNRG)
...
C  CANNOT USE PROPAGATOR SCRATCH FILE IF FIELDS ARE CHANGING
 ...
 1301         FORMAT(2X,59('= '))
              WRITE(6,1302) INRG,ENERGY(INRG),UNAME
...
C  ADD EREF
C
              ETEMP=ENERGY(INRG)*EFACT
              IF (IREF.NE.0 .OR. MONQN(1).NE.-99999)
     1          CALL THRESH(X(ISEINT),N,CINT,ITYPE,MONQN,NQN,
     2                      NJLQN(ITYP),EREF,X(ISJIND),IPRINT)
              ERED=ETEMP+EREF
              IF (IPRINT.GE.6) CALL EREFMS(EREF,EFACT,UNAME,MONQN,NQN)
              IF (IPRINT.GE.8) CALL EABSMS(ERED,EFACT,UNAME)
C
              ERED=ERED*CINT
...
C
     ...
     2                      NJLQN(ITYP),EREF,X(ISJIND),IPRINT)
              ERED=ETEMP+EREF
              IF (IPRINT.GE.7) CALL EREFMS(EREF,EFACT,UNAME,MONQN,NQN)
              IF (IPRINT.GE.8) CALL EABSMS(ERED,EFACT,UNAME)
C
              ERED=ERED*CINT
              CALL BDPROP(N,NSQ,MXLAM,NPOTL,X(ISYOUT),X(ISYIN),
...
C
     ...
     1                     'NODE',INODE,'HAS FAILED'
                IF (IPRINT.GE.8) THEN
                  WRITE(6,*) 'LATEST VALUES FOR FIELDS AND EIGENVALUES',
     1                       ' ARE',FLDLO(INODE),EIGLO(INODE),
     2                              FLDHI(INODE),EIGHI(INODE)
                ENDIF
                GOTO 500
              ENDIF
...
C
     ...
     2                      CONVGE)
              IF (IPRINT.GE.8) CALL EABSMS(ERED/CINT,EFACT,UNAME)
...
C
     ...
     1                      X(ISJIND),X(ISL),ERED/CINT,EREF,EFACT,UNAME)
              ENDIF

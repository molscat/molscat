...
C
C  REMAINING CODE IS FOR BACK-PROPAGATION OF WAVEFUNCTIONS
C
      IF (.NOT.WAVE) RETURN
...
C  LARGE NUMBER FORWARDS TO EVEN BIGGER NUMBER (WRITE OUT OF FIRST RECORD
C  IN SECOND CALL TO EFPROP IS SKIPPED OVER).
        JPREC=KREC(1)
        DO JSEG=JSEGB,JSEGE,-1
          IF (IPRINT.GE.8) WRITE(6,130) RESEG(JSEG),RBSEG(JSEG),
     1                                  NSTEPS(JSEG)
  130     FORMAT('  BACK-PROPAGATING WAVEFUNCTION FROM ',F10.4,' TO ',
     1           F10.4,' IN ',I6, ' STEPS'/)
          RSTART=RBSEG(JSEG)
          RSTOP=RESEG(JSEG)
...
C  COLLECT THE CONTRIBUTIONS TO THE NORMALISATION CONSTANT
 ...
 140  FORMAT(/"  NORMALISATION FACTOR CALCULATED BY SIMPSON'S RULE ",
     1        "FOR EACH CHANNEL")
      ANORM=0.D0
      DO I=1,N

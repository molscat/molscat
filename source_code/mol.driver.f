...
C  ENERGIES IN DATA FILE ARE IN UNITS SPECIFIED BY EUNITS
C  BUT ARE OFTEN WRITTEN OUT IN CM-1
C  DISTANCES IN DATA FILE ARE IN UNITS SPECIFIED BY RM (RETURNED
C  FROM POTENL).  CROSS SECTIONS, SCATTERING LENGTHS ETC
...
C  ILDSVU ADDED OCT 2012 BY GCM FOR WRITING LOG DERIVATIVE MATRIX TO FILE
C  SEE LDRDWT.F FOR DETAILS
...
C
     ...
     2                 DFIELD, DNRG,   DR,     DRAIRY, DRLR,
     3                 DRMAX,  DRNOW,  DTOL,   ENERGY, EREF,
     4                 EUNITS, FIELD,  FIXFLD, FLDMAX, FLDMIN,
     5                 IABSDR, IALFP,  IALPHA, IBFIX,  IBHI,
     6                 ICHAN,  ICONV,  IDIAG,  IFCONV, IFEGEN,
     7                 IFIELD, IFLS,   IFVARY, ILDSVU, IMGSEL,
     8                 INTFLG, IPARTU, IPERT,  IPHSUM, IPRINT,
     9                 IPROPL, IPROPS, IPSI,   IPSISC, IREF,
     A                 IRMSET, IRSTRT, IRXSET, ISAVEU, ISCRU,
     B                 ISHIFT, ISIGPR, ISIGU,  ISYM,   IV,
     C                 IVP,    IVPP,   IWAVE,  IWAVSC, IZERO,
     D                 JSTEP,  JTOTL,  JTOTU,  KSAVE,  LABEL,
     D                 LASTIN, LINE,   LMAX,   LOGNRG, LTYPE,
     E                 MAGEL,  MAXSTP, MHI,    MMAX,   MONQN,
     F                 MSET,   MXPHI,  MXSIG,  NCAC,   NFIELD,
     G                 NFVARY, NGAUSS, NGMP,   NLPRBR, NNRG,
     H                 NNRGPG, NTEMP,  NUMDER, OTOL,   PHILW,
     I                 PHIST,  POWRX,  PRNTLV, RMAX,   RMID,
     J                 RMIN,   RVFAC,  RVIVAS, SCALAM, SHRINK,
     K                 STABIL, STEPS,  STEST,  TEMP,   THETLW,
     L                 THETST, TOL,    TOLHI,  TOLMAX, TOLMIN,
     M                 URED,   VTOL,   WKBMN,  WKBMX,  XSQMAX
...
C  MAXSTP IS MAX NO. OF STEPS IN RADIAL PROPAGATION (INTFLG=3 ONLY)
C
      DATA CURRWD /'       ','(8-BYTE)'/
      DATA CTIME /'         '/,CDATE /'           '/
      DATA IPROGM /17/, PDATE /'2019.0 (beta)'/
      DATA PLUR /' ','S'/
      DATA CDRIVE /'M'/
      DATA PTIME  /.FALSE./
C
 ...
 1011 FORMAT(/'  PHYSICAL CONSTANTS OBTAINED FROM NIST CODATA '
     1       'TABULATION OF ',I4)
      ...
      IF (NIPR.EQ.1 .OR. NIPR.EQ.2) THEN
        WRITE(6,1200) MX,CURRWD(NIPR),AMXMB
 1200   FORMAT(/'  MEMORY ALLOCATED TO MAIN WORKING ARRAY IS',I10,1X,A8,
     1         ' WORDS (',F8.2,' MB)')
        WRITE(6,1202) NIPR,PLUR(NIPR)
 1202   FORMAT(2X,I1,' INTEGER',A1,' CAN BE STORED IN EACH WORD.')
      ELSE
        WRITE(6,1204) NIPR
 1204   FORMAT(/' *** ILLEGAL NIPR =',I10)
      ENDIF
C
      IF (LOGNRG) DNRG=1.0D0

      IF (IPRINT.GE.1) WRITE(6,1658) IPRINT,ISIGPR
 1658 FORMAT(/'  PRINT LEVEL =',I3,'     OTHER PRINT CONTROLS',
     1       '  ISIGPR =',I2)

      IF (IPRINT.GE.1) WRITE(6,1130) LABEL
 1130 FORMAT(/'  RUN LABEL = ',A80)

      IF (IPRINT.GE.1) WRITE(6,1470) URED
 1470 FORMAT(/'  REDUCED MASS FOR INTERACTION =',F14.9,
     1        ' ATOMIC MASS UNITS')

      IF (ISCRU.EQ.0) THEN
        IF (NNRG.GT.1. OR. NTEMP.GT.0) WRITE(6,1430)
 1430   FORMAT(/'  ***** WARNING - NO SCRATCH FILE SPECIFIED BY ISCRU ',
     1         'PARAMETER - FULL CALCULATION WILL BE DONE AT EACH ',
     2         'ENERGY')
      ELSEIF (ISAV.EQ.-1) THEN
          IF (IPRINT.GE.1) WRITE(6,1440) ISCRU
 1440     FORMAT(/'  ENERGY-INDEPENDENT MATRICES SAVED FROM A ',
     1           'PREVIOUS RUN WILL BE READ FROM UNIT',I3)
          OPEN(ISCRU,FORM='UNFORMATTED',STATUS='OLD')
      ELSE !IF (ISAV.EQ.0) THEN
        IF (IPRINT.GE.1) WRITE(6,1450) ISCRU
 1450   FORMAT(/'  ENERGY-INDEPENDENT MATRICES WILL BE SAVED ',
...
C
      CALL ECNV(EUNITS,EFACT,UNAME,IPRINT)

      IF (IPRINT.GE.1) WRITE(6,1660)
 1660 FORMAT(/2X,59('=='))
C
C--------------------------------------------------------------------
      ...
      IF (NEXTRA.NE.0) THEN
        IF (IPRINT.GE.1) THEN
          IF (EFACT.NE.1.D0) THEN
            WRITE(6,1101) DEGTOL,DEGTOL/EFACT,UNAME
          ELSE
            WRITE(6,1101) DEGTOL
          ENDIF
          WRITE(6,1102) NEXTRA
...
C  TEST ON ICHAN, IFCONV AND NNRG TO IDENTIFY OTHER PROCEDURES
        IF (ICHAN.NE.0 .AND. IFCONV.EQ.-1 .AND. NNRG.EQ.1) THEN
          LSPEC=.TRUE.
        ENDIF
        IF (ICHAN.NE.0 .AND. IFCONV.GE.1  .AND. NNRG.EQ.1) THEN
          LCHAR=.TRUE.
        ENDIF
      ENDIF
      ...
      IF (LSPEC .AND. FLDMAX.LE.FLDMIN) THEN
        ...
        LSPEC=.FALSE.
      ENDIF
 1666 FORMAT(/2X,'*** WARNING: ',A,' CANNOT BE PERFORMED BECAUSE ',
     1       'FLDMAX <= FLDMIN.',/,
     2       /6X,'ONE CALCULATION WILL BE PERFORMED (AT FLDMIN).')
      IF (LCHAR .AND. FLDMAX.LE.FLDMIN) THEN
        WRITE(6,1666) 'RESONANCE CHARACTERISATION'
        IFCONV=0
        LCHAR=.FALSE.
      ENDIF
...
C
        IF (NNRG.LE.0) THEN
          WRITE(6,1540)
 1540     FORMAT(/' ***** ERROR - NO INPUT ENERGIES SPECIFIED - ',
     1           'RUN HALTED')
          STOP
        ENDIF
C
        IF (NNRG.GT.1 .AND. DNRG.GT.0.D0 .AND. ICONV.EQ.0) THEN
          IF (LOGNRG) THEN
...
C
      IF (NUMDER .AND. IPRINT.GE.1) WRITE(6,1656)
 1656 FORMAT(/'  NUMDER=.TRUE.  POTENTIAL DERIVATIVE WILL BE COMPUTED',
     &       ' NUMERICALLY FROM POTENTIAL.')
      ILAM=IXNEXT
      MXLAM=NIPR*(MX-ILAM+1)
      CALL POTENL(-1,MXLAM,X(ILAM),RM,EPSIL,ITYPE,IPRINT)
C  THIS READS (5, POTL).  RM AND EPSIL ARE SET HERE.
C  RM (SPECIFIED IN A) IS USED FOR ALL LENGTH INPUTS AND MOST OUTPUTS
C  EPSIL (SPECIFIED IN CM-1) IS USED ONLY FOR VALUES FOR POTENL
      WRITE(6,1661) RM
 1661 FORMAT(/'  ALL LENGTHS ARE IN UNITS OF RM =',F10.6,' ANGSTROM',
...
C
C  CINT CONVERTS ENERGIES IN CM-1  INTO UNITS OF 1/RM**2
      CINT=URED*RM*RM/BFCT
C  RMLMDA IS THE SQUARE OF THE RATIO OF RM TO DEBROGLIE WAVELENGTH
C  AND IS THE FACTOR USED TO CONVERT THE POTENTIAL INTO 1/R^2 UNITS
      RMLMDA=CINT*EPSIL
C
      IF (IPRINT.GE.1) WRITE(6,1660)
      CALL IFMSG(IPROPS,IPROPL,INTFLG,RMIN,RMAX,DRLR,RVFAC,
     1           IRMSET,IRXSET,SHRINK,NNRG,IABSDR,IMGSEL,IPRINT)
      ...
      IF (IWAVE.NE.0 .AND. .NOT.WAVE) THEN
        WRITE(6,*) '  WAVEFUNCTIONS ARE ONLY CALCULATED FOR ',
     &             'LDMD PROPAGATOR'
      ENDIF
...
C
     ...
     1                   10.D0**(-IRMSET)))**(2.D0/3.D0)
        IF (IPROPS.EQ.3 .AND. SHRINK.EQ.1) THEN
...
C
      ...
      ISCRU=IABS(ISCRU)
C
C  STOP IF BASE9 REQUIRES OPTIONS NOT IMPLEMENTED IN PROPAGATOR
C
      LDIAGH=(IPROPS.EQ.-1 .OR. IPROPS.EQ.2 .OR. IPROPS.EQ.3 .OR.
     1        IPROPL.EQ.-1 .OR. IPROPL.EQ.2 .OR. IPROPL.EQ.3)
      IF ((NCONST.GT.0 .OR. NRSQ.GT.0) .AND. LDIAGH) THEN
        WRITE(6,1665) IPROPS,IPROPL
 1665   FORMAT(/' IPROPS AND IPROPL =',I2,' & ',I2/
     1         ' THESE ARE NOT COMPATIBLE WITH A BASE9 ROUTINE ',
     2         'THAT SETS NCONST OR NRSQ > 0'/
     3         ' USE LOG-DERIVATIVE PROPAGATOR(S). STOPPING.')
        STOP
      ENDIF
...
C
      IF (IPRINT.GE.1) THEN
        ...
        ELSEIF (EUNITS.LE.9) THEN
          WRITE(6,1013) TRIM(UNAME),EUNITS
        ELSE
          WRITE(6,1014) TRIM(UNAME),EUNITS
        ENDIF
 ...
 1014   FORMAT(/'  INPUT ENERGY VALUES CONVERTED FROM ',A/
     ...
     1                .AND. ICONV.EQ.0 .AND. IPRINT.GE.1)
     2    WRITE(6,1510) (ENERGY(I),I=1,NNRG)
 1510   FORMAT(/'  INPUT ENERGY LIST IS'/1P,(16X,7G16.6))
      ENDIF
C
      IF (ICONV.EQ.0) THEN
        IF (IPRINT.GE.1) WRITE(6,1600) NNRG,PLUR(MIN(NNRG,2))
 1600   FORMAT(/'  CONTROL DATA FOR TOTAL ENERGIES.  CALCULATIONS ',
     1         'WILL BE PERFORMED FOR',I4,' VALUE',A1)
        IF (IPRINT.GE.1) THEN
          DO I=1,NPR
            IF (EFACT.NE.1.D0) THEN
              WRITE(6,1620) I,ENERGY(I),ENERGY(I)/EFACT,UNAME
            ELSE
              WRITE(6,1620) I,ENERGY(I)
            ENDIF
          ENDDO
        ENDIF
 1620   FORMAT(7X,'ENERGY NO.',I4,' =',1PG19.11,' CM-1',:' =',
     &         G19.11,1X,A8)
      ELSE
        IF (NNRG.LE.1) THEN
     ...
     1               'TESTING'
        ...
        IF (IPRINT.GE.1) THEN
          IF (EFACT.NE.1.D0) THEN
            WRITE(6,1621) NNRG,ENERGY(1),ENERGY(1)/EFACT,UNAME
          ELSE
            WRITE(6,1621) NNRG,ENERGY(1)
          ENDIF
        ENDIF
 1621   FORMAT(/'  CONTROL DATA FOR CONVERGENCE TESTS.  CALCULATIONS ',
     1         'WILL BE PERFORMED ',I4,' TIMES'/
     2         7X,'ENERGY =',1PG19.11,' CM-1',:' =',G19.11,1X,A)
      ENDIF
C
      IF (ISRCH.EQ.1 .AND. IPRINT.GE.1) WRITE(6,1630)
 1630 FORMAT(/'  RESONANCE SEARCH OPTION. ONLY FIRST 5 ENERGIES ',
     1       'GIVEN. OTHERS WILL BE DETERMINED INTERACTIVELY.')
C
      IF (IFEGEN.GT.0 .AND. IPRINT.GE.1) WRITE(6,1640)
 1640 FORMAT(/'  THESE ENERGY VALUES WILL BE USED AS KINETIC ENERGIES',
...
C  WRITE HEADER
        IF (IFCONV.EQ.-1) THEN
          WRITE(6,101) '  PROGRAM WILL ATTEMPT TO CONVERGE ON ',
     1                 'SCATTERING LENGTH A = AZERO FOR CHANNEL',
     2                 ICHAN,' TO ACCURACY',DTOL,TRIM(EFVUNT(ISVEFV)),
     3                 '  BETWEEN FLDMIN AND FLDMAX FOR ',
     4                 TRIM(EFVNAM(ISVEFV)),
     5                 '  WITH AZERO =',AZERO
        ELSEIF (IFCONV.GE.1) THEN
          WRITE(6,101) '  PROGRAM WILL ATTEMPT TO CHARACTERISE ',
     ...
     4                 TRIM(EFVNAM(ISVEFV))
          ...
          ELSEIF (IFCONV.EQ.2) THEN
            WRITE(6,102) '  USING FORMULA INCORPORATING WEAK '//
     1                   ' BACKGROUND INELASTICITY.',TOLMIN,TOLMAX
          ELSEIF (IFCONV.EQ.3) THEN
            WRITE(6,102) '  USING FULL TREATMENT OF INELASTIC '//
     1                   'RESONANCE',TOLMIN,TOLMAX
          ENDIF
        ENDIF
  101   FORMAT(/A,A,I4,A,1PG10.2,' ',A/A,A/:A,F8.3/)
  102   FORMAT(A/'  TOLMIN = ',F6.4,' AND TOLMAX = ',F6.4)
...
C  RESET VALUES READY FOR LOOP LATER ON
      ...
      IF (IPRINT.GE.1) THEN
        WRITE(6,1660)
        IF (JTOTL.LT.JTOTU) THEN
          WRITE(6,1480) JTOTL,JTOTU,JSTEP
        ELSE
          WRITE(6,1481) JTOTL
        ENDIF
        IF (JTOTU.GE.99999) THEN
          WRITE(6,1490) NCAC,DTOL,OTOL
          IF (NNRGPG.GT.1) WRITE(6,1491) NNRGPG
        ENDIF
      ENDIF
 1480 FORMAT(/'  TOTAL ANGULAR MOMENTUM JTOT RUNS FROM',I4,'  TO',
     1        I6,'  IN STEPS OF',I4)
 1481 FORMAT(/'  TOTAL ANGULAR MOMENTUM JTOT =',I4)
 1490 FORMAT(/'  JTOT SERIES WILL BE TERMINATED WHEN MAX CHANGE IN ',
     1       'CROSS SECTIONS IS LESS THAN TOLERANCE FOR NCAC =',I3,
     2       ' CONSECUTIVE JTOT'/25X,'DIAGONAL (DTOL) AND ',
     3       'OFF-DIAGONAL (OTOL) TOLERANCES ARE',2F9.5)
 1491 FORMAT(/'  N.B. CONVERGENCE CHECKING IS DONE FOR ENERGY GROUPS',
     1       ' OF NNRGPG =',I4)

      IF (IPRINT.GE.1)
     1  WRITE(6,'(/2X,A,I3,A)') 'EACH JTOT IS SPLIT INTO A MAXIMUM OF ',
     1                          MXPAR,' DIFFERENT SYMMETRY BLOCKS'
      IF (IBFIX.GT.0 .AND. IBHI.GT.0 .AND. IBHI.LT.IBFIX) IBHI=0
      IF (IBFIX.GT.0 .AND. IBHI.LE.0) IBHI=IBFIX
      IF (IBFIX.EQ.0) IBHI=MXPAR
      IF (IBFIX.GT.0 .AND. IPRINT.GE.1) THEN
        IF (IBFIX.LT.IBHI) THEN
          WRITE(6,1500) IBFIX,IBHI
        ELSE
          WRITE(6,1501) IBFIX
        ENDIF
      ENDIF
 1500 FORMAT(/'  CALCULATIONS WILL BE FOR SYMMETRY BLOCKS',
     1       I4,'  TO',I4)
 1501 FORMAT(/'  CALCULATION(S) WILL BE FOR SYMMETRY BLOCK',I4)
...
C
     ...
     4              DEEP,IPRINT,NSTATE,ISAVEU,TFIRST,RM,EPSIL,RMIN,
     5              RMAX,MONQN,IBOUND,WAVE)
        CALL GCLOCK(TLAST)
        TOTIME=TLAST-TFIRST
        GOTO 9020
      ENDIF
...
C
      IF (IFLS.GT.0) THEN
        CALL PRBRIN(IFLS,LINE,LTYPE,ILSU,NNRG,ENERGY,IFEGEN,
     1              X(ISJSTT),IPRINT)
        IF (IFEGEN.GT.0) NPR=NNRG
        IF (IPRINT.GE.1) WRITE(6,1660)

        IF (IPHSUM.NE.0) THEN
          WRITE(6,1680) IFLS,IPHSUM
 1680     FORMAT(/'  ****** WARNING. IFLS =',I3,' AND IPHSUM =',I3,
...
C
      IF (NNRG*NFIELD.GT.MXNRG) THEN
        WRITE(6,1691) NNRG,NFIELD,MXNRG
 1691   FORMAT('  NNRG =',I5,' * NFIELD =',I5,' WOULD OVERFLOW ',
...
C  LEVELS WILL BE CONSISTENT BETWEEN DIFFERENT CALCULATIONS)
      IF (NFVARY.EQ.1 .AND. IFVARY(1).EQ.0) THEN
        LACCXS=LCURXS
      ELSE
        LACCXS=(LCURXS .AND. NFVARY.EQ.0)
      ENDIF

      CALL OUTINT(LABEL,ENERGY,EFACT,NNRG,NFIELD,NSTATE,NQN,X(ISJSTT),
...
C  PROCESS RESTART REQUEST ...
     ...
     2              ENERGY,X(IOUT),X(IACC),X(IDEG),ISST,IECONV,
     3              MINJT,MAXJT,ISIGU,IPARTU,OTOL,DTOL,X(IC1),X(IC1),
     4              MRSTRT,IERST,IFST,MXP,IPRINT,LCURXS,LACCXS,
     5              ICHAN,NSIG,NFIELD,IBOUND,CINT)
      ENDIF
      IF (IPRINT.GE.1) WRITE(6,1660)
C
...
C  EINT AND JSINDX ARE NOT DEFINED YET BUT ARE NOT NEEDED IN THIS CALL
     ...
     1  CALL THRESH(X(1),N,CINT,ITYPE,MONQN,NQN,NJLQN(ITYP),EREF,X(1),0)
      IF (IPRINT.GE.1) CALL EREFIN(MONQN,NQN,NJLQN(ITYP),UNAME,EREF,
     1                             EFACT)

      CALL GCLOCK(TITIME)
      TTIME=TITIME-TFIRST
      IF (IPRINT.GE.1) WRITE(6,1700) TTIME,NUSED
 1700 FORMAT(/'  INITIALIZATION DONE.  TIME WAS',F7.2,' CPU SECS.',I10,
...
C
      ...
      DO 100 JTOT=JTOTL,JTOTU,JSTEP
        IF (IPRINT.GE.2) WRITE(6,1730) JTOT
 1730   FORMAT(/2X,43('*'),'  ANGULAR MOMENTUM JTOT  =',I4,'  ',
     1              43('*'))
...
C
          IF (IRSTRT.GE.2 .AND. JTOT.EQ.JTOTL .AND. IB.LT.MRSTRT) THEN
            WRITE(6,1736) IB,IRSTRT
 1736       FORMAT('  *** SKIPPING SYMMETRY BLOCK NO =',I3,
     1             '  DUE TO IRSTRT =',I3)
            GOTO 200
          ENDIF
C
...
C
     ...
     3                EFIRST,NSTATE,IPRINT,IBOUND,X(ISJSTT),X(ISDGVL))
          ENDIF
C
C  LOOP OVER EXTERNAL FIELDS OR SCALING FACTOR IF REQUESTED
C
          OLDFAC=0.D0
          IFXE=0
...
C  QUANTITY IF SEARCH OR CHARACTERISATION OF RESONANCE BEING PERFORMED
     ...
     1          IB.EQ.MRSTRT .AND. JFIELD.LT.IFST) THEN
              WRITE(6,1780) JFIELD,IRSTRT
 1780         FORMAT('  *** SKIPPING EFV SET NO =',I3,
     1             '  DUE TO IRSTRT =',I3)
              GOTO 300
            ENDIF
...
C
     ...
     2                      X(ISNEVR),IBOUND,EFACT,UNAME)
              ELSEIF (IPRINT.GE.6) THEN
                CALL THRLST(N,X(ISEINT),X(ISCENT),CINT,X(ISL),
     2                      IBOUND,EFACT,UNAME)
              ENDIF

C
C  CHECK THAT RMAX IS BEYOND CENTRIFUGAL BARRIER
              CALL FINDRX(ENERGY,X(ISEINT),X(ISCENT),NPR,N,CINT,
     1                    RMAX,RSTOP,EREF,NOPMAX,IRXSET,IPRINT)
C
C  IF RMIN IS FIXED (NOT CALLING FINDRM) CAN SET PROPAGATOR ENDPOINTS NOW
              IF (IRMSET.EQ.0)
     1          CALL PRCONT(RMIN,0.D0,0.D0,0.D0,RSTOP,0.D0,0)
            ENDIF
C
C ******************  LOOP OVER ENERGIES BEGINS HERE  ******************
C
            E4WMAX=ENERGY(1)+EREF*CINT
            IF (ISCRU.GT.0) E4WMAX=EMX+EREF*CINT

            CALL WVCALC(X(ISWVEC),WMAX,E4WMAX,X(ISEINT),NOPEN,N)
...
C
              ...
              DO J=JLO,JHI
                ...
                IF (IECONV(IFXE).LT.0 .AND. IECONV(IFXE).GT.-2*MXP) THEN
                  SFX=ORDNL(J)
                  WRITE(6,1778) JTOT,J,SFX
 1778             FORMAT(/'  * * * WARNING.  JTOT =',2I5,'-',A2,
...
C
     ...
     2              J.LT.IERST) THEN
                  WRITE(6,1781) J,IRSTRT
 1781             FORMAT('  *** SKIPPING ENERGY NO =',I3,
     1                   '  DUE TO IRSTRT =',I3)
                  GOTO 500
                ENDIF
C
C  ADDRESS OF CURRENT SET OF ACCUMULATED CROSS SECTIONS:
                ICXS=IOUT+((JFIELD-1)*NNRG+(J+1))*NSTOR
...
C
                IF (IFLS.GT.0 .AND. IFEGEN.GE.2) THEN
                  CALL PRBCNT(J,X(ISJIND),N,IUSE)
                  IF (IUSE.EQ.0) THEN
                    LWARN=.TRUE.
                    IF (IPRINT.GE.4) WRITE(6,1786) JTOT,IB,J,ENERGY(J)
 1786               FORMAT(/'  ****** S MATRIX FOR JTOT =',I5,
...
C
                IF (IPRINT.GE.2) THEN
                  IF (IEL.NE.1 .OR. J.NE.JLO .OR. JFIELD.NE.1)
     1              WRITE(6,FMT=F711) TRIM(LABEL)
                  IF (EFACT.NE.1.D0) THEN
                    WRITE(6,1788) JTOT,IB,J,ENERGY(J),ENERGY(J)/EFACT,
     1                            TRIM(ADJUSTL(UNAME))
                  ELSE
                    WRITE(6,1788) JTOT,IB,J,ENERGY(J)
                  ENDIF
 1788             FORMAT(/'  JTOT =',I5,'   IBLOCK =',I4,
...
C
                ETOT=ENERGY(J)+EREF
                IF (IPRINT.GE.4) CALL EREFMS(EREF,EFACT,UNAME,MONQN,NQN)
                ERED=ETOT*CINT
C
                IF (ICODE.EQ.0) THEN
                  EFIRST=ERED
...
C
                IF (ITYPE.EQ.8 .AND. J.NE.1) THEN
                  ...
                  IF (IPRINT.GE.2) WRITE(6,1798) J,ETOT,THETJ
 1798             FORMAT(/'  NOTE: K VECTORS PARALLEL TO SURFACE WERE ',
...
C
     ...
     2                      ICODE,ISAV,EFIRST,IPRINT)
C
                IF (ICODE.EQ.1 .AND. IRMSET.GT.0) THEN
C  FOR IRMSET > 0 OPTION, CHOOSE APPROPRIATE RMIN
                  RSTART=RMIN
                  E4RMIN=ERED
                  IF (ITYPE.NE.8) E4RMIN=EMX+EREF*CINT
                  CALL FINDRM(X(ISSI),N,RSTART,RTURN,IK,X(ISP),X(ISVL),
     1                        X(ISIV),E4RMIN,X(ISEINT),X(ISCENT),RMLMDA,
     2                        X(IT1),X(IT2),X(IT3),X(IT4),MXLAM,NPOTL,
     3                        IRMSET,ITYPE,IPRINT)
C
C  USE ALTERED VALUES FOR RMIN (RSTART), RMAX (RSTOP) TO CALCULATE VALUE
C  FOR RMID AND HENCE CHOOSE ENDPOINTS FOR ALL PROPAGATORS
                  CALL PRCONT(RSTART,RTURN,RSTOP,0.D0,RMAX,RVFAC,IPRINT)
                ELSE
                  RTURN=RMIN
                  IK=1
                ENDIF
                CALL PRCHCK(STEPS,LSKIP)
                IF (LSKIP) THEN
                  IF (IPRINT.GE.4) WRITE(6,1799) JTOT,IB,J
 1799             FORMAT(2X,'SKIPPING JTOT =',I5,' IBLOCK =',
...
C  BE REINSTATED IF NECESSARY TO READ OLD FILES.
                ...
                IF (ILDSVU.GT.0) THEN
                  IDUM = LDRWPH(ILDSVU,.TRUE., JTOT, J,ENERGY(J), IEXCH,
     1                          WT, IB, N, ERED,RMLMDA)
                ENDIF

C
C  SOLVE COUPLED EQUATIONS.
C  PROPAGATORS ARE CALLED FROM SUBROUTINE SCPROP

                IF (WAVE) THEN
                  CALL WVINFO(JTOT,IB,ICHAN,N,NQN,NSTATE,X(ISJSTT),
     1                        X(ISJIND),X(ISL),ETOT,EREF,EFACT,UNAME)
                ENDIF
                CALL SCPROP(N,MXLAM,NV,NPOTL,
     1                      X(ISJIND),X(ISSR),X(ISSI),X(ISK),X(ISVL),
     2                      X(ISIV),X(ISEINT),X(ISCENT),X(ISWVEC),
     3                      X(ISL),X(ISNB),X(ISP),DEGTOL,NOPMAX,DEEP,
     4                      IK,ICODE,IPRINT,IBOUND,ICHAN,WAVE,WMAX)
C
                CALL GCLOCK(TJTIME)
                TTIME=(TJTIME-TITIME)
                TITIME=TJTIME
...
C  RESET ICODE TO ALLOW "SUBSEQUENT ENERGY" CALCULATIONS
                ...
                ELSE
                  IF (IPRINT.GE.2) THEN
                    WRITE(6,1900) JTOT,IB,J,ENERGY(J)
                    IF (PTIME) WRITE(6,1901) TTIME
                  ENDIF
 1900             FORMAT(/'  ****** NO OPEN CHANNELS FOR JTOT =',I5,3X,
     1                   'IBLOCK =',I4,'   ENERGY(',I4,') =',
     2                   1PG19.11)
 1901             FORMAT(2X,'STEP TIME =',0PF6.2,' SECS')
                  IF (IECONV(IFXE).GE.0) IECONV(IFXE)=IECONV(IFXE)+1
                  GOTO 500
                ENDIF
C
C  FORCE IRSTRT=0  SO THAT ISAVEU WILL BE UPDATED.
                IRSTRX=0
                IF (ICONV.GT.0) CALL CONVRG(J,X(ISSR),X(ISSI),X(ISSRO),
     1                                      X(ISSIO),IPRINT)
C
     ...
     1                      X(ISWVEC),X(ISSR),X(ISSI),X(ISK),SCLEN,
     2                      CONV,NOPEN,IB,MXPAR,WGHT,IEXCH,J,TTIME,
     3                      ENERGY(J),EREF,X(IOUT),X(ICXS),X(IDEG),
     4                      X(ISJSTT),ISST,IECONV(IFXE),MINJT(IFXE),
     5                      MAXJT(IFXE),NSTATE,
     5                      NQN,OTOL,DTOL,IPHSUM,ISIGU,IPARTU,ISAVEU,
     6                      ISIGPR,IRSTRX,ICHAN,X(IT1),X(IT2),X(ISCENT),
     7                      X(ISEINT),IBOUND,X(IT1),X(IT2),X(ISNEVR),
     8                      CINT,JFIELD,IPRINT,.FALSE.,PTIME)
                IXNEXT=IT1
C
...
C
                IF (ICHAN.GT.NOPEN) THEN
                  WRITE(6,1910) ICHAN,NOPEN
 1910             FORMAT(/'  *** WARNING ICHAN = ',I3,' IS GREATER'
...
C
                  ...
                  IF (JFIELD.EQ.2) THEN
                    IF (SCLNOW*SCLLST.GT.0.D0) THEN
                      WRITE(6,1920) FLDMIN,FLDMAX,SCLNOW
 1920                 FORMAT(/'  A - AZERO HAS SAME SIGN AT FLDMIN =',
     ...
     3                       '. ABANDON CONVERGENCE ATTEMPT.')
                  ...
                  IF (JFIELD.GE.2) THEN
                    ...
                    ELSEIF (JFIELD.GT.2) THEN
                      ...
                      ENDIF
                    ENDIF
                    FLDNEW=BRENT(XA,XB,XC,FA,FB,FC,JFIELD.EQ.2,DTOL,
     1                           CONVGE,METHOD)
                    DFSTEP=FLDNEW-FLDNOW
                    IF (.NOT.CONVGE) THEN
                      IF (IPRINT.GE.4) WRITE(6,1767) METHOD,SCLNOW,
     1                  TRIM(EFVNAM(ISVEFV)),FLDNEW,
     2                  TRIM(EFVUNT(ISVEFV)),DFSTEP
 1767                 FORMAT(/'  ',A,' STEP FROM A - AZERO=',G12.4,
     1                       ' GIVES ',1X,A,' =',F13.8,1X,A,
     2                       ' WITH STEP SIZE',G12.4)
                    ELSE
                      IF (IPRINT.GE.1) WRITE(6,1768)
     1                  TRIM(EFVNAM(ISVEFV)),FLDNEW,
     2                  TRIM(EFVUNT(ISVEFV)),DFSTEP
 1768                 FORMAT('  CONVERGED ON A = AZERO AT ',A,' = ',
...
C
                IF (J.EQ.1) THEN
                  XK2REF=CINT*ENERGY(J)
                  SCLREF=DBLE(SCLEN)
                ELSE
                  XK2NOW=CINT*ENERGY(J)
                  SCLNOW=DBLE(SCLEN)
     ...
     1                     (XK2NOW-XK2REF)
                    ...
                    IF (IPRINT.EQ.1) CALL MSGEFV(JFIELD,NFIELD)
                    WRITE(6,1775) ICHAN
                    WRITE(6,1776) '1/A',J,RNGEFF,TRIM(RMUNIT),
     1                            '1/A',SCLZER,TRIM(RMUNIT)//'^-1'
                    RNGEFF=2.D0*(SCLNOW-SCLREF)/(XK2NOW-XK2REF)
                    SCLZER=SCLREF-0.5D0*XK2REF*RNGEFF
                    RNGEFF=RNGEFF/SCLZER**2
                    WRITE(6,1776) ' A ',J,RNGEFF,TRIM(RMUNIT),
     1                            '  A',SCLZER,TRIM(RMUNIT)
 1775               FORMAT(/'  EFFECTIVE RANGE FOR CHANNEL',I4,':')
 1776               FORMAT('  FROM EXPANSION OF ',A3,
     2                     ' AT ENERGIES 1 AND',I4,' IS',G13.5,1X,A,
     3                     ' WITH ',A3,'(K=0) =',G19.10,1X,A)
                  ENDIF
                ENDIF
C
                IF (IECONV(IFXE).GE.0 .AND. IFLS.GT.0) THEN
C
...
C
                IF (IPRINT.GE.5) THEN
                  WRITE(6,1950) JTOT,IB,J,ENERGY(J)
                  IF (PTIME) WRITE(6,1901) TTIME
                ENDIF
 1950           FORMAT(/'  FINISHED JTOT =',I5,'   IBLOCK =',I4,
     1                 '   ENERGY(',I4,') =',1PG14.7)
C
  500         CONTINUE
C
...
C
              IF (ISRCH.NE.0) THEN
                CALL NEXTE(ENERGY(JLO),EPSM,ENEW,DNRG,EFACT,UNAME,
     1                     IPHSUM,IPRINT)
                IF (JHI.NE.NNRG) THEN
C  PROTECT AGAINST NEGATIVE ENERGIES UNLESS IREF IS IN USE
                  IF (ENEW.LE.0.D0 .AND. IIREF.EQ.0) GOTO 9000
                  JST=JHI+1
                  JND=MIN(JHI+NNRGPG,NNRG)
                  IF (IPRINT.GE.1) WRITE(6,1600) NNRG,PLUR(MIN(NNRG,2))
                  DO JJ=JST,JND
                    ENERGY(JJ)=ENEW+(JJ-JST)*DNRG
C                   ENEV=ENERGY(JJ)/8065.541D0
                    IF (IPRINT.GE.1) THEN
                      IF (EFACT.NE.1.D0) THEN
                        WRITE(6,1620) JJ,ENERGY(JJ),ENERGY(JJ)/EFACT,
     1                                UNAME
                      ELSE
                        WRITE(6,1620) JJ,ENERGY(JJ)
                      ENDIF
                    ENDIF
                  ENDDO
                ENDIF
...
C
          IF (IPRINT.GE.1) THEN
            IF (LSPEC) WRITE(6,1971) NFIELD
 1971       FORMAT(/,'  CONVERGENCE ON VALUE NOT ACHIEVED IN',I4,
     1             ' STEPS')
            IF (LCHAR) WRITE(6,1972) NFIELD
 1972       FORMAT(/,'  RESONANCE CHARACTERISATION NOT ACHIEVED IN',I4,
     1             ' STEPS')
          ENDIF
C
  290     CALL RSTEFV(FIXFLD)
          IF (ALDONE) THEN
            IF (IPRINT.GE.1) WRITE(6,1969) DTOL,OTOL,NCAC
 1969       FORMAT(///'  CALCULATION TERMINATED BY CONVERGENCE OF ',
     1             'TOTAL CROSS SECTIONS.'//'  DIAGONAL AND OFF-',
     2             'DIAGONAL TOLERANCES WERE ',2F9.5,'   NCAC =',I3)
            GOTO 9000
          ENDIF
C
          IF (IPRINT.GE.2) WRITE(6,1660)
...
C
        IF (IFLS.GT.0) CALL PRBOUT(JSTEP,JTOT)
  100 CONTINUE
C
...
C
C  CALL TO OUTPCH NEEDS FIXING TO HANDLE GENERALISED FIELD LOGIC
 9000 IF (.NOT.LSPEC .AND. .NOT.LCHAR)
     1  CALL OUTPCH(X(IACC),ENERGY,NNRG,FIELD,NFIELD,MINJT,MAXJT,
     2              ISIGPR,LABEL,F710,UNAME,ISIGU,LWARN,EREF,X(ISINLV),
     3              X(ISNEVR),IPRINT)
      IF (IFLS.GT.0 .AND. IPRINT.GE.1) WRITE(6,FMT=F710) TRIM(LABEL)
      IF (IFLS.GT.0) CALL PRBOUT(JSTEP,-99999)

...
C
 ...
 1430   FORMAT(/'  NO SCRATCH FILE SPECIFIED BY ISCRU PARAMETER: ',
...
C
 ...
 1600   FORMAT(/'  CONTROL DATA FOR TOTAL ENERGIES.  CALCULATIONS ',
     1         'WILL BE PERFORMED FOR',I4,' VALUE',A1)
        IF (IPRINT.GE.1) THEN
          DO I=1,NPR
            IF (EUNIT.NE.1.D0) THEN
              WRITE(6,1620) I,ENERGY(I),ENERGY(I)/EUNIT,EUNAME
            ELSE
 ...
 1620   FORMAT(7X,'ENERGY NO.',I4,' =',1PG19.11,' CM-1',:' =',
     ...
     1               'TESTING'
        ...
        IF (IPRINT.GE.1) THEN
          IF (EUNIT.NE.1.D0) THEN
            WRITE(6,1621) NNRG,ENERGY(1),ENERGY(1)/EUNIT,EUNAME
          ELSE
...
C
     ...
     1              WRITE(6,FMT=F711) TRIM(LABEL)
                  IF (EUNIT.NE.1.D0) THEN
                    WRITE(6,1788) JTOT,IB,J,ENERGY(J),ENERGY(J)/EUNIT,
     1                            TRIM(ADJUSTL(EUNAME))
                  ELSE
...
C  FORCE IRSTRT=0  SO THAT ISAVEU WILL BE UPDATED.
                IRSTRX=0
                IF (NCONV.GT.0) CALL CONVRG(J,X(ISSR),X(ISSI),X(ISSRO),
...
C
     ...
     2                      SCLEN,NOPEN,IB,MXPAR,WGHT,IEXCH,
     3                      J,TTIME,ENERGY(J),EREF,X(IOUT),X(ICXS),
...
C  TEMPORARY STORAGE FOR PRBR -- THESE ARE INTEGERS, COULD USE NIPR
                  ...
                  IXNEXT=IT4+N
                  CALL CHKSTR(NUSED)
                  CALL PRBR(JTOT,MOLD,NOPEN,J,RUNIT,
...
C
              IF (ISRCH.NE.0) THEN
                CALL NEXTE(ENERGY(JLO),EPSM,ENEW,DNRG,EUNIT,EUNAME,
     1                     IPHSUM,IPRINT)
                IF (JHI.NE.NNRG) THEN
...
C                   ENEV=ENERGY(JJ)/8065.541D0
                    IF (IPRINT.GE.1) THEN
                      IF (EUNIT.NE.1.D0) THEN
                        WRITE(6,1620) JJ,ENERGY(JJ),ENERGY(JJ)/EUNIT,
     1                                EUNAME
                      ELSE

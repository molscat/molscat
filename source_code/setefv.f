...
c     IF (NEFV.EQ.-1) THEN
c       WRITE(6,9002) '*** ERROR: NUMBER OF EXTERNAL FIELD VARIABLES '//
c    1                '(NEFV) HAS NOT BEEN SET IN BAS9IN.'
 9002   FORMAT(/2X,A)
c       STOP
c     ELSEIF (NEFV.LT.-1) THEN
      IF (NEFV.LT.0) THEN
...
C  CHECK WHETHER IFIELD HAS BEEN SET
c     IF (IFIELD.NE.-1) THEN
c       WRITE(6,9004) '*** WARNING: IFIELD IS NO LONGER USED.',
c    1                'IFIELD = ',IFIELD,' WILL BE IGNORED.'
c     ENDIF

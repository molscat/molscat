...
C  AMENDED BY CR Le Sueur 2018 ish
      ...
      USE efvs, ONLY: EFV, EFVNAM, EFVUNT, IEFVST, ISVEFV, LEFVN,
     e                LEFVU, LISTFV, MAPEFV, MXEFV, NEFV, NEFVP, NNZRO,
     e                SCALAM, SVNAME, SVUNIT
...
C  CHARACTER VARIABLES TO SET UP FORMAT STATEMENTS
     ...
     1        NFVARY,IFVARY(MXEFV),IUSE,IFIELD
      ...
      IF (LFRSET) THEN
        IF (NFIELD.GT.1 .AND. FLDMAX.NE.FLDMIN .AND.
     1      FLDMAX.NE.0.D0) THEN
          DFIELD=(FLDMAX-FLDMIN)/(NFIELD-1)
        ELSEIF (NFIELD.GT.1) THEN
          FLDMAX=FLDMIN+(NFIELD-1)*DFIELD
        ENDIF
        IF (DFIELD.NE.0.D0) THEN
...
C  (ONLY RELEVANT IF IPRINT < 10)
      ...
      IF (IPRINT.LT.10 .AND. MAPEFV.GE.0) THEN
        ...
        IF (MAPEFV.GE.0 .AND. IFVARY(1).GE.0) THEN
          DO IEFV=1,NEFV
            IF (FIXSET(IEFV) .AND. FIXEFV(IEFV).NE.0.D0)
     1        NONZRO(IEFV)=.TRUE.
          ENDDO
          DO IVARY=1,NFVARY
            IEFV=IFVARY(IVARY)
...
C  REINIV MAY ALSO READ IN AND USE ALTERED VALUES OF PARAMETERS.
C  INTRODUCED BY JMH NOV 2019
C  THE DEFAULT VERSION OF REINIV DOES NOTHING.
C  ORIGINALLY CALLED ONLY IF SCALAM IS IN USE:
C     IF (IFVARY(1).EQ.0 .OR. SCALAM.NE.1.D0) CALL REINIV(SCALAM)
C  BUT NOW CALLED UNCONDITIONALLY TO ALLOW READING OF NEW PARAMETERS
C  EVEN IF THIS PARTICULAR RUN DOES NOT USE SCALAM.
      CALL REINIV(SCALAM)

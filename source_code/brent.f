...
C  IF |DB| >= |(XC-XE)/2| OR |XE| < |DTOL| USE BISECTION
     ...
     1    ABS(DB).GE.ABS(CME)/2D0 .OR. ABS(XE).LT.ABS(DTOL)) THEN
      ...
      BRENT=XB+DB
      XD=XC
C  UNLIKE THE WIKIPEDIA PAGE/NUMERICAL RECIPES, CRLS THINKS THE TEST USED
C  TO DECIDE WHETHER TO ACCEPT INTERPOLATION (|DB| < |(XC-XE)/2|),
C  WHERE XC, XE ARE THE TWO PREVIOUS RESULTS, SHOULD ENFORCE THAT XC AND
C  XE BRACKET THE ROOT.  THIS SEEMS TO WORK BETTER.
      IF (FC*FB.GT.0.D0) THEN
        XC=XA
        FC=FA
      ELSE
        XC=XB

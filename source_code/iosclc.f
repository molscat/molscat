     ...
     1                  ITYPX,RMIN,RMAX,DEEP,IRMSET,IRXSET,RVFAC,
...
C
     ...
     1                  RMAX,RSTOP,EREF,NOPMAX,IRXSET,IOSPR)
C
...
C  GET TEMPORARY STORAGE FOR FINDRM; MODIFED 23 AUG FOR NEW FINDRM (SG)
            ...
            CALL FINDRM(X(IXSR),NVC,RSTART,RTURN,IK,X(IT1),X(IXVL),
...
C  RELEASE TEMPORARY STORAGE
            IXNEXT=IT2
C
C  NOW READY TO SOLVE 'COUPLED' EQUATIONS; DONE AS CALL TO SCPROP.
C
 3005       NV=NPOTL*NVC*(NVC+1)/2
            CALL PRCONT(RSTART,RTURN,RSTOP,0.D0,RMAX,RVFAC,IOSPR)
     ...
     1                                     ' BECAUSE RMIN > RMAX'
            ...
            IXNEXT=ICX+MXLAM
            CALL CHKSTR(NUSED)
            CALL SCPROP(NVC,MXLAM,NV,NPOTL,
     1                  X(IXJIND),X(IXSR),X(IXSI),X(IXKMAT),X(IXVL),
     +                  X(IXIV),X(IXEINT),X(IXCENT),X(IXWV),
     2                  X(IXLORB),X(IXNB),X(ICX),DEGTOL,NOPMAX,DEEP,
     3                  IK,ICODE,IOSPR,IBOUND,1,WAVE,WMAX)
            IXNEXT=ICX
C

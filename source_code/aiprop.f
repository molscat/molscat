...
C
     ...
     :         SYMINV, SPROPN, DSCAL, TRNSP, WAVEIG
      DIMENSION Z(*),VL(*),IV(*),EINT(*),CENT(*)
      ALLOCATABLE :: Y1(:),Y2(:),Y4(:),VECNOW(:),VECNEW(:),
     1               EIGOLD(:),EIGNOW(:),HP(:),TMAT(:),W(:),P(:)
...
C
      ...
      ALLOCATE(W(NCH*NCH),EIGOLD(NCH),HP(NCH),TMAT(NCH*NCH),
...
C
      ALLOCATE (VECNOW(NCH*NCH),VECNEW(NCH*NCH),EIGNOW(NCH),Y1(NCH))
      ALLOCATE (Y2(NCH),Y4(NCH))
...
C
      ...
      DEALLOCATE (Y1,Y2,Y4,TMAT,W,VECNOW,VECNEW,EIGOLD,EIGNOW,HP,P)
C

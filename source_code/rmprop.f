      SUBROUTINE RMPROP(N,MXLAM,NHAM,
     1                  RMAT,VL,IV,EINT,CENT,
...
C
      ...
      INTEGER IPRINT
      ALLOCATABLE :: W(:),VECNOW(:),VECOLD(:),EIGOLD(:),EIGNOW(:),
     1               DIAG(:),R1(:),R2(:),R3(:),R4(:),Q(:),P(:)
      DIMENSION VL(*),IV(*),EINT(N),CENT(N),RMAT(N*N)
C
...
C
      NSQ=N*N
      ALLOCATE(W(NSQ),DIAG(N),EIGOLD(N),VECOLD(NSQ),Q(NSQ),
     1         P(MXLAM+NCONST+NRSQ))
      IF (IREAD) GOTO 170
C
      RNOW=RSTART+0.5D0*DR
      DRNOW=DR
      DRNEW=DRNOW
      CALL HAMMAT(W,N,RNOW,P,VL,IV,ERED,EINT,CENT,EP2RU,CM2RU,
...
C  PROPAGATE R MATRIX
      ALLOCATE(EIGNOW(N),VECNOW(NSQ),R1(N),R2(N),R3(N),R4(N))
...
C  IF ICODE = 1 CALCULATE EIGNOW AND Q MATRIX
  250   CALL HAMMAT(W,N,RNOW,P,VL,IV,ERED,EINT,CENT,EP2RU,CM2RU,
     1              RSCALE,DIAG,MXLAM,NHAM,IPRINT)
C
        IFAIL=0
...
C  REACH HERE WHEN ASYMPTOTIC REGION IS REACHED
  450 CALL TRNSP(VECOLD,N)
      ...
      DEALLOCATE(W,VECOLD,VECNOW,EIGOLD,EIGNOW,DIAG,R1,R2,R3,R4,Q,P)

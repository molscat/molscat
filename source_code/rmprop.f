      SUBROUTINE RMPROP(N,NSQ,MXLAM,NPOTL,
...
C
      IF (IREAD) GOTO 170
C
C  LOGIC CONTROLLED BY VARSTP ADDED NOV 18 TO PROPAGATE EXACTLY TO RSTOP
C  USING EITHER A FIXED STEP OR A STEP PROPORTIONAL TO R
C  (INITIAL STEP SIZE IS NOW CALCULATED IN DRSET)
C
      RNOW=RSTART+0.5D0*DR
      DRNOW=DR
      DRNEW=DRNOW
      CALL WAVMAT(W,N,RNOW,P,VL,IV,ERED,EINT,CENT,RMLMDA,DIAG,
     1            MXLAM,NPOTL,IPRINT)
      IFAIL=0
      CALL F02ABF(W,N,N,EIGOLD,VECOLD,N,R1,IFAIL)

      SUBROUTINE HAMMAT(W,N,R,P,VL,IV,ERED,EINT,CENT,EP2RU,CM2RU,
...
C
      DIMENSION W(N,N),VL(*),IV(*),EINT(N),CENT(N),P(*),DIAG(N),
     1          IDUM1(1)
C
...
C
C  REPLACE ANY EXCESSIVELY LARGE VALUES TO MAINTAIN STABILITY WHEN USED
C  THIS IS PARTICULARLY IMPORTANT IF ANY COEFFICIENTS ARE INFINITE
C
      DO I=1,MXLAM
        IF (ABS(P(I)).GT.1.D60) THEN
          P(I)=SIGN(1.D60,P(I))
          IF (IPRINT.GE.15) WRITE(6,600) R,I,P(I)
  600     FORMAT('  WARNING IN HAMMAT AT R =',F10.4,' FOR CHANNEL',
     1           I4,': EXCESSIVE POTENTIAL COEFFICIENT REDUCED TO ',
     2           1PE7.1)
        ENDIF
      ENDDO
C
...
C
      CALL PERTRB(R*RSCALE,P,MXLAM+NCONST+NRSQ,0)
C

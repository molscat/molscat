...
C
      ...
      DAMP6(X) =(1.D0-EXP(-X*(A6 +X*(B6 +D6*X))))**6
      DAMP8(X) =(1.D0-EXP(-X*(A8 +X*(B8 +D8*X))))**8
      DAMP10(X)=(1.D0-EXP(-X*(A10+X*(B10+D10*X))))**10
      CORR(X)=1.D0+GMU*EXP(-GEX*X)
      C2PHI=2.D0*COS(PHI)**2-1.D0
      C4PHI=2.D0*C2PHI*C2PHI-1.D0
      CTH2=CTH*CTH
      STH2=1.D0-CTH2
      F3=CTH*STH2*C2PHI
      F40=(3.D0-CTH2*(30D0-35.D0*CTH2))/8.D0
      F44=STH2*STH2*C4PHI*SQRT(35.D0/64.D0)
      F4=SQRT(7.D0)*F40-SQRT(5.D0)*F44
C  Assumes FACE is -root(1/3),0; VERTEX is -root(1/3),90; EDGE is 1,0
      C6A=C6
      C7A=C7*F3/2.D0
      C8A=C8
      C8A=C8A+RAT8*C8*F4*0.4D0/SQRT(7.D0)
      C9A=C9*F3/2.D0
      ...
      X=RM/RA
      Y=X*X
      REP=SCALE*EXP(-ALPHA*R)*(1.D0+0.67D0*F3*SQRT(13.125D0/PI)

...
C  SET THE PROPAGATION LIMITS IN DRIVE TO THE NEW VALUES
      IF (ICON.EQ.0 .AND. STEPS.LE.0.D0) THEN
        IF (DRS.NE.UNSET) DRS=DRS*2.D0
      ELSEIF (ICON.EQ.0 .AND. STEPS.GT.0.D0) THEN
        STEPS=STEPS/2.D0
      ENDIF
      IF (ICON.EQ.0 .AND. STEPL.LE.0.D0) THEN
        IF (DRL.NE.UNSET .AND. DRL.NE.-1.D0) DRL=DRL*2.D0
      ELSEIF (ICON.EQ.0 .AND. STEPL.GT.0.D0) THEN
        STEPL=STEPL/2.D0
      ENDIF
      IF (ICON.EQ.1) RMXINT=RMXINT-DRCON
      IF (ICON.EQ.2) RMNINT=RMNINT+DRCON
      RETURN
C

...
C  INCLUDING BOTH DIAGONAL AND OFF-DIAGONAL TERMS
      IBSFLG=2
      READ(5,BASIS9)
      WRITE(6,107) IS,JMAX,LMAX
 107  FORMAT(/'  BASIS SETS USE DIATOM SPIN =',I2/
     1       17X,'NMAX =',I3/17X,'LMAX =',I3)
...
C  NL IS NUMBER OF SETS OF INTERNAL QUANTUM NUMBERS FOR THIS LEVEL
  ...
  210 CONTINUE
      GOTO 230
  220 WRITE(6,602)
  602 FORMAT(/'  BASIS FUNCTIONS TAKEN FROM &BASIS (JLEVEL) INPUT')
C
...
C
  ...
  250 CONTINUE
      ...
      IF (EIN) THEN
         WRITE(6,605)
  605    FORMAT(/'  ENERGY LEVELS TAKEN FROM &BASIS (ELEVEL) INPUT')
      ELSE
         WRITE(6,604) (ROTI(I),I=1,3)
  604    FORMAT(/'  ENERGY LEVELS CALCULATED FROM:'/'  Bv     =',F10.5/
     &    '  GAMMA  =',F10.5/'  LAMBDA =',F10.5)
      ENDIF
      DO 270 I=1,NLEVEL
         NN=JSTATE(I)
         JJ=JSTATE(I+NSTATE)
...
C  PARITY CHECK
  ...
  320 CONTINUE
C
      IF (LCOUNT) WRITE(6,608) M,IBLOCK-1
...
C  NEXT IF EXPLOITS SOME 3-J SYMBOLS PROPERTIES
              IF (IABS(MLAMB).GT.LAMB .OR. MLAMB.NE.(MJCOL-MJROW) .OR.

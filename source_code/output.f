      SUBROUTINE OUTPUT(JTOT, NBASIS, JSINDX, L, INDLEV, WVEC,
     1                  SREAL, SIMAG, AKMAT, SCLEN, CONV, NOPEN,
     2                  IB, MXPAR, WGHT, IEXCH, INRG, TTIME,
     3                  ENERGN, EREF, SIGCUR, SIGACC, SIGDEG,
     4                  JSTATE, ISST, JECONV, MINJTN, MAXJTN, NSTATE,
     5                  NQN, OTOL, DTOL, IPHSUM, ISIGU,IPARTU,ISAVEU,
     6                  ISIGPR,IRSTRT, ICHAN, TEMP1, TEMP2, CENT, EINT,
     7                  IBOUND,INDUSE,INDACC,LNEVER,CINT,JFIELD,IPRINT,
     8                  LRESRT,PTIME)
...
C                 TO FIX BUG W/ NO BASIS FNS FOR SYMMETRY BLOCK IB=MXPAR
...
C
     ...
     1        LLOPEN,LUSE,LNEVER(1),LNEG,LDIAG,LRESRT,PTIME
...
C  SET LOGICAL VARIABLES
      OKEY=CONV.GE.0.D0 .AND. JECONV.GE.0
...
C  PRINT OUT OPEN-CHANNEL BASIS FUNCTIONS AND WAVEVECTORS IN 1/A.
      IF (IPRINT.GE.10) THEN
        WRITE(6,'(/2X,A)') 'OPEN CHANNEL LIST:'
        IF (IBOUND.EQ.0) THEN
          NAMEL='    L '
        ELSE
          NAMEL='<L**2>'
...
C  RESTORE WVEC TO UNITS OF 1./RM
      DO I=1,NOPEN
        WVEC(NBASIS(I))=WVEC(NBASIS(I))*RM
      ENDDO
...
C  SORT THE LIST
  ...
  191   FORMAT(/4X,'F',2X,'I =',10(4X,I3,8X))
  192   FORMAT( 2X,I3, 5X,   1P,10(E13.5,2X))
  193   FORMAT(/2X,60('* '))

        DO ISET=1,(IUSE-1)/NCOLS+1
          WRITE(6,191) (INDUSE(I),I=(ISET-1)*NCOLS+1,
     1                              MIN(ISET*NCOLS,IUSE))
          DO I=1,IUSE
            IT=INDUSE(I)
            WRITE(6,192) IT,(ABS(SIGCUR((INDUSE(II)-1)*NSIG+IT)),
...
C
 3200 IF (IPRINT.GE.1) THEN
        IF (IPRINT.LT.2 .OR. IPRINT.GE.5) THEN
...
C
      IF (ISIGPR.GT.0 .AND. IPRINT.GE.11) THEN ! .AND. JTOT.NE.MINJTN) THEN

        WRITE(6,210) MINJTN,JTOT
  210   FORMAT(/2X,11('* '),'STATE-TO-STATE INTEGRAL CROSS SECTIONS ',
     1         'ACCUMULATED FROM JTOT =',I4,' TO',I4,12(' *'))
        ISTART=(INRG+1)*NSTOR
C
        IF (JHALF.EQ.0) THEN
          XJS=1.D0
  ...
  220     FORMAT(31X,'MULTIPLIED BY',F5.1,' TO ACCOUNT FOR JSTEP.')
        ENDIF
C
        IUSE=0
        ...
        DO ISET=1,(IUSE-1)/NCOLS+1
          WRITE(6,191) (INDACC(I),I=(ISET-1)*NCOLS+1,
     1                              MIN(ISET*NCOLS,IUSE))
          DO I=1,IUSE
            IT=INDACC(I)
            WRITE(6,192) IT,(ABS(SIGACC((INDACC(II)-1)*NSIG+IT))*XJS,
     1                       II=(ISET-1)*NCOLS+1,MIN(ISET*NCOLS,IUSE))
          ENDDO
        ENDDO
        WRITE(6,193)
...
C  IN CASE THERE ARE NO BASIS FNS FOR SYMMETRY BLOCK IB=MXPAR.
C
      ENTRY OUTSIG(ISIGU,IB,MXPAR,INRG,ENERGN,MINJTN,MAXJTN,SIGACC,
     1             IPRINT)
...
C  UPDATE DISK (ISIGU) RECORD IF THIS IS THE LAST SYMMETRY BLOCK
 4000 IF (LOUT .AND. IB.EQ.MXPAR) THEN
        XJS=JTSTEP
        IJ=0 !(INRG+1)*NSTOR
        I10=ISST(INRG)
        DO 4100 I=1,NSIG
...
C  06-09-2018: THIS WRITE STATEMENT ADDED
          WRITE(ISAVEU) NDGVL,NCONST,NRSQ,IBOUND
...
C  FORMAT FOR SCATTERING LENGTH
     ...
     1        "X,'CHAN   L POW',5X,'WVEC*"//ADJUSTL(RMUNIT)//
     2        "',7X,'RE(A)/"//ADJUSTL(RMUNIT)//"',7X,"//
     3        "'IM(A)/"//TRIM(ADJUSTL(RMUNIT))//"')"
...
C
 ...
 9200     CONTINUE
          IF (ISIGPR.GT.0 .AND. IOPEN.GT.1) THEN
            WRITE(6,950)

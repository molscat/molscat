...
C  FOR MOLSCAT, ALL PROPAGATIONS ARE TOWARDS RMAX
     ...
     1              'IN',NSEG,'SEGMENT',PLUR(MIN(NSEG,2))
        WRITE(6,*)
        IF (NSEG.EQ.1) THEN
          WRITE(6,*) ' PROPAGATION RANGE IS CONTROLLED BY VARIABLES '//
     1               'RMIN AND RMAX, WITH INPUT VALUES'
          WRITE(6,920) ' RMIN = ',RMIN,' RMAX = ',RMAX
  920     FORMAT(1X,A,1PG11.4,A,G11.4:,A,G11.4:,A,G11.4)
        ELSE
          WRITE(6,*) ' PROPAGATION RANGES ARE CONTROLLED BY '//
     1               'VARIABLES RMIN, RMID AND RMAX, WITH INPUT VALUES'
          WRITE(6,920) ' RMIN = ',RMIN,' RMID = ',RMID,' RMAX = ',RMAX
        ENDIF
C
C  FOR BOUND/FIELD, ALL PROPAGATIONS ARE TOWARDS RMATCH
     ...
     1              'RMATCH IN',NSEG,'SEGMENTS'
        ...
        IF (NSEG.EQ.2) THEN
          WRITE(6,*) ' PROPAGATION RANGES ARE CONTROLLED BY '//
     1               'VARIABLES RMIN, RMATCH AND RMAX, WITH INPUT '//
     2               'VALUES'
          WRITE(6,920) ' RMIN = ',RMIN,' RMATCH = ',RMATCH,
     1                 ' RMAX = ',RMAX
        ELSE
          WRITE(6,*) ' PROPAGATION RANGES ARE CONTROLLED BY '//
     1               'VARIABLES RMIN, RMID, RMATCH AND RMAX, WITH '//
     2               'INPUT VALUES'
          WRITE(6,920) ' RMIN = ',RMIN,'RMID = ',RMID,
...
!'  NOTE THAT THIS IS IMPLEMENTED ONLY FOR ONE CHANNEL CASES, E.G., IOS CALCULATIONS.'//
  ...
  320     FORMAT(/'  COUPLED EQUATIONS SOLVED BY PROPAGATOR OF ',
     1           'DEVOGELAERE'/'  PROPAGATION PARAMETERS ARE: ',3X,
...
C  TEXT ABOUT HOW STEP LENGTH IS CHOSEN
  ...
  400   FORMAT(/'  PROPAGATION STEP SIZE DETERMINED USING ',A,' = ',
     ...
     1               ' ACCURACY VIA PERTURBATION THEORY'
          IF (IPROP.EQ.IAIRY) WRITE(6,900) TOLHIT,POWRX
  900     FORMAT(2X,'WITH TOLHI = ',1PG9.2:,' AND POWR = ',G9.2)
        ELSE
          IF (POWR.EQ.1.D0) THEN
            WRITE(6,*) ' STEP SIZE CONSTANT THROUGHOUT RANGE'
          ELSE
            WRITE(6,910) POWR
  910       FORMAT(2X,'STEP BOUNDARIES ARE EQUALLY SPACED IN R**(1/',
     1               F5.2,')')
          ENDIF

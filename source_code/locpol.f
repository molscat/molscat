...
C  FIRST, SORT 3 EXISTING POINTS
      ...
      IF (IDECAY.EQ.1) THEN
        ...
        IF (IPRINT.GE.6) WRITE(6,170) IMIN,TRIM(C140),ARES,RUNAME,
  ...
  120   FORMAT('  PREDICTED STEP MORE THAN 10 TIMES ',A,' RANGE'/
     1       '  PROBABLY OSCILLATING SO ABANDON CONVERGENCE')
      ...
      ENDIF
      IF (IDECAY.EQ.0) THEN
C  IF IDECAY=0, CHECK THE RESONANCE ISN'T ACTUALLY DECAYING.
...
C  THESE CRITERIA HASVEN'T BEEN TESTED, JUST A FIRST GUESS AT THIS STAGE.
        IF (ABS(DIFF).GT.MAX(0.01D0*ABS(SCLNRE(IMIN)),1.D0)) THEN
          WRITE(6,125) DIFF
...
C
C  ON EXIT: SRBG AND SIBG ARE THE REAL AND IMAGINARY PARTS OF THE S-MATRIX
C           ELEMENT FAR FROM RESONANCE
C           DIAMRE AND DIAMIM ARE THE REAL AND IMAGINARY PARTS OF THE DIAMETER
C           OF THE CIRCLE IN THE COMPLEX PLANE DESCRIBED BY THE S-MATRIX
C           ELEMENT ACROSS THE RESONANCE
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      X1=2.D0*(B1-BRES)/GAM
      X2=2.D0*(B2-BRES)/GAM

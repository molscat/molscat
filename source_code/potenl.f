...
C  -----------------------------------------------------------------
      NAMELIST/POTL/ A,     CFLAG, E,     EPSIL, ICNSYM,
     1               ICNSY2,IHOMO, IHOMO2,IVMIN, IVMAX,
     2               LAMBDA,LMAX,  L1MAX, L2MAX, LVRTP,
     3               MMAX,  MXLAM, MXSYM, NPOTL, NPOWER,
     4               NPS,   NPTS,  NPT,   NTERM, RM
C
...
C  -------- BELOW DESCRIBE TERMS AS EXPONENTIALS/INVERSE POWERS
...
C>>IT WOULD MAKE GOOD SENSE TO USE IVMIN,IVMAX TO GENERATE HERE.
 ...
 3020       FORMAT('  *** WARNING.  INPUT LEGENDRE LABEL IS > ZERO ',
     1             'BELOW.  LABEL WILL BE IGNORED AND SET TO ZERO.')
            LAM(3*I-2)=0
            IF (IPRINT.GE.1)
 ...
 9353           FORMAT(/'  COEFFICIENTS IN POTENTIAL WILL BE ',
     ...
     2                 'POTENTIAL INTO CORRECT FORM')
        ...
        ELSEIF (ITYP.EQ.8) THEN
          ...
          QNAME(2)=QTYPE(12)
c         NPOTL=NPTL
c         WRITE(6,9380) NPOTL
c9380     FORMAT('  SURFACE FOURIER COMPONENTS'/'  NPOTL =',I2,
c    1           ' FROM SURBAS')
        ENDIF
...
C  PROCESS EXPRESSIONS FOR EXPANSION COEFFICIENTS
 ...
 9997       FORMAT(/'  *** POTENL.  DIMENSION ',A,' EXCEEDED',I6)
            STOP
          ENDIF
          IF (NPOWER(IX).GT.0) THEN
            WRITE(6,9510) IX,NPOWER(IX)
 9510       FORMAT(/'  * * * WARNING - POSITIVE EXPONENT OF R ILLEGAL',
     1             2I6/'  NEGATIVE OF SUPPLIED VALUE ASSUMED')
 ...
 6000 IF (IPRINT.GE.1) WRITE(6,9990) EPSIL,RM
 9990 FORMAT(/'  POTENL PROCESSING FINISHED.'//
     1       '  UNITS FOR POTENTIAL ROUTINES:'/
     2       '  POTENTIAL RETURNED IN UNITS OF EPSIL =',F15.6,' CM-1'/
     3       '  R                  IN UNITS OF RM    =',F15.6,
     4       ' ANGSTROMS')
C
      R=RM
      P(1)=EPSIL
      IF (RM.EQ.1.D0) RMUNIT='ANGSTROM'
      IF (EPSIL.EQ.1.D0) EPUNIT='CM-1'
c     MPOTL=NPOTL
      MXLMB=MXLAM
      RETURN
C

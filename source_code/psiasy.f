...
C  THIS VERSION USES SIMPLE EXPONENTIALS, NOT RICCATI-HANKEL FUNCTIONS, SO IT
...
C  TO BE TRANSFORMED TO THE PRIMITIVE BASIS FOR SOME USES.
      ...
      PARAMETER (IU = (0.D0,1.D0))
      DOUBLE COMPLEX TERM

      PIO2 = 0.5D0*ACOS(-1.D0)
...
C  COMPUTE OUTGOING WAVE COMPONENT IN OPEN CHANNELS
      DO I = 1, NOPEN
         ...
         TERM = (-1.D0/SQRT(WK))*EXP(IU*(WK*REND-PIO2*WL))*
...
C  HERE SO WE CAN USE ORDINARY INDEXING INTO WVEC AND L.
      WK = WVEC(LAMBDA)
      WL = L(LAMBDA)
      IF (WK.GE.0.D0) THEN !INCOMING CHANNEL OPEN; OK
         TERM = (1.D0/SQRT(WK))*EXP(-IU*(WK*REND-PIO2*WL))
...
C  18 APRIL 2013 G. C. MCBANE.
C
C  ALTERNATE VERSION THAT USES RICCATI-HANKEL FUNCTIONS
...
C  COMPUTE OUTGOING WAVE COMPONENT IN OPEN CHANNELS
      DO I = 1, NOPEN
         ...
         HLP = IU*ZJN-ZYN  ! RICCATI-HANKEL H_L^+ =  I*JHAT - YHAT
C  S-MATRIX IS ONLY DEFINED BETWEEN OPEN CHANNELS
         TERM = (-1.D0/SQRT(WK))*HLP*(SR(I,NL0)+IU*SI(I,NL0))
         PSIRE(NX) = DREAL(TERM)
         PSIIM(NX) = DIMAG(TERM)
     ...
     1       I, NX, PSIRE(NX), PSIIM(NX), SR(I,NL0), SI(I,NL0)
         ...
         IF (NX.EQ.LAMBDA) THEN  ! THIS IS INITIAL CHANNEL;
            ...
            HLM = -IU*ZJN-ZYN ! RICCATI-HANKEL H_L^- =  -I*JHAT - YHAT
            TERM = (1.D0/SQRT(WK))*HLM
...
C  ABRAMOWITZ AND STEGUN CALLS Y.
      ...
      DO I = 1, NOPEN
         ...
         WKR = WK*REND
         CALL RBESJY(DBLE(L(NX)),WKR,ZJN,ZJNP,ZYN,ZYNP)
         TERM = (1.D0/SQRT(WK)) * ZYN * K(I,NL0)
         WRITE(6,*) ' TERM1 = ', TERM
         PSIRE(NX) = TERM

         IF (NX.EQ.LAMBDA) THEN  ! THIS IS SELECTED CHANNEL; ADD J PART
            TERM = (1.D0/SQRT(WK))*ZJN
            WRITE(6,*) ' TERM2 = ', TERM
            PSIRE(NX) = PSIRE(NX) + TERM
         ENDIF
         WRITE(6,'(2X,I1,1X,5(E12.4,1X))') I, 1.D0/SQRT(WK),

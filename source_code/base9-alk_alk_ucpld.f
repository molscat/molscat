...
C  FULLY UNCOUPLED BASIS MSA MIA MSB MIB
...
C  DEC 13 ALTERED TO INCLUDE EXTRA OPERATORS
C
...
C  ALTERED TO USE ARRAY SIZES FROM MODULE sizes ON 23-06-17 BY CRLS
     ...
     2                  NEXTRA
C  IN OLD CODE BASIS9 ALSO HAS LMIN, WHICH IS NEEDED TO WORK OUT LSTART
C
...
C  SET UP ELEMENTS OF efvs MODULE
      ...
      EFVUNT(1)='GAUSS'
C  POSITION OF MAGNETIC FIELD IN VCONST ARRAY
      MAPEFV=2
...
C
cINOLLS include 'base9/alk_alk-namelist.h'
...
C
      MXPAR=2
C  LOOPS USING IBLOCK RATHER THAN JTOT REQUIRE THIS LINE INSTEAD
C     IFMAX=INUCA+ISA+INUCB+ISB
C     MXPAR=1+IFMAX/2+LMAX
      NLABV=1
      NQN=5
...
C
C  IN OLD CODE THE LINES FOLLOWING (UPTO MTOT=JTOT) MUST BE REPLACED
C  BY THE FOLLOWING
C     IFMAX=INUCA+INUCB+ISA+ISB
C     MTOT=MOD(IFMAX,2)+2*(IBLOCK-1)
C
...
C
      IF (LCOUNT) THEN
C
C  IN OLD CODE THIS WRITE STATEMENT IS REPLACED BY:
C       WRITE(6,605) (-1)**IPAR,IBLOCK,MTOT
C 605   FORMAT('  PARITY',I3,/'  IBLOCK =',I3,' SELECTS MTOT =',I2,'/2')
...
!  Start of long DO loop #1
      DO I=1,NSTATE
        ...
        MF=MSA+MSB+MIA+MIB
        ML=MTOT-MF
C  IN OLD CODE, LSTART IS SET UP AS FOLLOWS:
C     LSTART=MAX(2*LMIN,ABS(ML))
C     IF (2*(LSTART/4).NE.LSTART/2-IPAR) LSTART=LSTART+2
C     IF (IDENTN .AND. MSA.EQ.MSB .AND. MIA.EQ.MIB
C    1   .AND. 2*(LSTART/4).NE.LSTART/2-IBOSFR) LSTART=2*LMAX+1
        LSTART=4-2*IBLOCK
C
...
C
      IF (LCOUNT) RETURN
C
C  IN OLD CODE CHANNELS ARE NOT SORTED
C
...
!  Start of long IF block #1
          IF (LL.LE.NSPIN .AND. LR.EQ.LC .AND. MLR.EQ.MLC) THEN
            XS=SPMIN+DBLE(LL-1)

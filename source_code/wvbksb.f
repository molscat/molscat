      SUBROUTINE WVBKSB(N, RBEGIN, REND, NSTEP, DR, PSIB,
...
C  (SEE NUMERICAL RECIPES EQN 4.1.14)
        IF (.NOT.LREGSP) THEN
          A=1.D0
        ELSEIF (ISTEP.EQ.1 .OR. ISTEP.EQ.NSTEP+1) THEN
          A=17.D0/48.D0
...
C
        IF (ISTEP.NE.1 .OR. LFIRST) THEN
          ROLD=R
          IF (.NOT.LREGSP .AND. ISTEP.NE.1) R=RP
...
c       IF (LREGSP .AND. ABS(RP-R).GT.1.D-8) THEN
c         WRITE(6,*)' EFPROP: MISMATCH WITH R READ FROM FILE AT STEP =',
c    1              ISTEP,', R = ',R,' AND RP (FROM FILE) = ',RP
c         STOP
c       ENDIF
        IPREC=IPREC-IDIR
C

...
C
      DIMENSION NBASIS(*),JSINDX(*),L(*),JSTATE(NSTATE,NQN)
      DIMENSION WVEC(*),SREAL(*),SIMAG(*)
      ALLOCATABLE :: IC(:),IC1(:),IL(:),IL1(:),SRTEMP(:),SITEMP(:)
C
...
C
      ...
      ALLOCATE (IC(N),IC1(N),IL(N),IL1(N))
...
C  FOR DIAG CASE (JA=JA1), IC/IC1 AND IL/IL1 HAVE SAME VALUES.
          ...
          DO II=1,N
            ...
            IC(NLVAL)=II
            IL(NLVAL)=L(JJ)
 3201       IF (JSINDX(JJ).NE.LN(I,IA+7)) CYCLE
            NLVAL1=NLVAL1+1
            IC1(NLVAL1)=II
            IL1(NLVAL1)=L(JJ)
          ENDDO
...
C  ALLOCATE TEMPORARY STORAGE FOR SR,SI,TR,JBAR,ISTB,NBLK,LVAL
          NSQ=N*N
          NINTS=(N+NIPR-1)/NIPR
          ALLOCATE (SRTEMP(NSQ),SITEMP(NSQ))
          CALL PRBR3(N,SREAL,SIMAG,JTOT,NSTATE,NQN,JSTATE,NBASIS,
     1               JSINDX,L,NPACK,LN(I,IA),NLVAL,IC,IL,SRTEMP,SITEMP)
...
C<<SG 1 JUN 93
C
 3212     IF (NLVAL.LE.0 .OR. NLVAL1.LE.0) CYCLE
...
C  JJ LOOPS FOR L2 => FINAL (PRIMED) I.E. JA1:IL1,IC1  (ROWS)
            DO II=1,NLVAL
              IF (L1.NE.IL(II)) CYCLE
              DO JJ=1,NLVAL1
                IF (L2.NE.IL1(JJ)) CYCLE
                NMATCH=NMATCH+1
                IX=(IC(II)-1)*N+IC1(JJ)
...
C  FOR ITYPE3 RESTORE SREAL,SIMAG AND RELEASE TEMP STORAGE
          IF (ITYPE3) THEN
            CALL PRBR3R(N,SREAL,SIMAG,SRTEMP,SITEMP)
C  RELEASE TEMPORARY STORAGE
            DEALLOCATE (SRTEMP,SITEMP)
          ENDIF
C
        ENDDO
...
C  N.B. DIMENSION OF LINE PASSED FROM DRIVER IS 2*MXLN
 ...
 1100 IF (IPRINT.GE.1) WRITE(6,110)
  110 FORMAT(/5X,' LINE   LEV(A) LEV(B)  LEV(A1) LEV(B1)')
      DO I=1,NLPRBR
        IST=NPL*(I-1)
        IF (IPRINT.GE.1) WRITE(6,112) I,(LINE(IST+II),II=1,NPL)
  112   FORMAT(5X,I4,I8,I6,I9,I7)
      ENDDO
      NLINE=0
      NEWE=NNRG
!  Start of long DO loop #4
      LOOP_I: DO I=1,NLPRBR
        PF=.FALSE.
        L1=LINE((NPL*(I-1))+1)
        L2=LINE((NPL*(I-1))+2)
        IF (LDIAG) GOTO 1104
...
!  End of long DO loop #6
 1403   IF (NEPL.GT.0) CYCLE LOOP_I
...
C  REACH HERE IF IT IS NOT USED.  COMPRESS LIST & REVISE INDEXES
      ...
      DO I=1,NLINE
        ...
        IF (LN(I,5).GT.II) LN(I,5)=LN(I,5)-1
      ENDDO
      GOTO 1609
C
...
C
      DO I=1,NLINE
        SIGRJS=SIGR(I)*FACTJ
        SIGIJS=SIGI(I)*FACTJ
        WRITE(6,601) I,LN(I,1),LN(I,2),LN(I,8),LN(I,9),

      SUBROUTINE AIRPRP(NCH,MXLAM,NPOTL,
     1                  Z,TMAT,W,VL,IV,EINT,CENT,P,
     2                  Y1,Y2,CC,Y4,VECNOW,VECNEW,
     3                  EIGOLD,EIGNOW,HP,TOLAI,POWR,
     4                  RSTART,XF,REND,DRNOW,NSTEP,ERED,RMLMDA,
...
C
C  AIRY ZEROTH-ORDER PROPAGATOR FROM R=XF TO R=REND
...
C                    ON ENTRY Z CONTAINS THE INITIAL Z-MATRIX AT R=XF
C                    ON RETURN Z CONTAINS THE Z-MATRIX AT R=REND
C   W, TMAT, VECNOW
...
C                              DISTANCE
C                              THIS IS EQUAL TO REND IF NORMAL
C                              TERMINATION
...
C  TOLAI:            PARAMETER TO DETERMINE STEP SIZES
C                    IF TOLAI .LT. 1, THEN ESTIMATED ERRORS ARE USED TO
C                    DETERMINE NEXT STEP SIZES FOLLOWING THE PROCEDURE
C                    OUTLINED IN M.H. ALEXANDER, "HYBRID QUANTUM SCATTERING
C                    ALGORITHMS ..."
C                    IF TOLAI .GE. 1, THEN STEP SIZES ARE CONTROLLED BY
C                    GEOMETRIC SERIES
C                    ALGORITHM:  DRNEXT = TOLAI * DRNOW
...
C
      DRNORM=DRNOW
      IF (DRNOW.LT.0.D0 .AND. TOLAI.GT.1.D0 .AND. XF.LT.-1D-10)
     &THEN
        DRNOW=XF
      ENDIF
      NODES = 0
...
C
C  JMH BUG-FIX 15/11/2010: CHANGE 70 TO 61 TO INITIALIZE Y
C  EVEN WHEN NO SCRATCH FILE IS BEING WRITTEN OR READ
C  ORIGINAL MAXSTP FORMULA CAN OVERFLOW SO HARD-CODE IT. JMH 8/2012
C     MAXSTP = ( (REND-XF) / DRNOW ) * 20
      MAXSTP = 500000
...
C
C ----------------------------------------------------------------------
      IF (DRNORM.LT.0.D0 .AND. TOLAI.GT.1.D0) THEN
        DRNOW=DRNORM
      ENDIF
...
C
        IF (KSTEP.GT.1 .OR. ISTART.EQ.1) THEN
          CALL TRNSP(TMAT, NCH)
...
C
C  THIS IS ONLY DONE IF TOLAI .LT. 1, IN WHICH CASE THE LARGEST
        ...
        IF (TOLAI.LT.1.D0) THEN
          ...
          IF (IPRINT.GE.1) THEN
            IF (CMAX.GT.(5.D0 * TOLAI)) THEN
              WRITE(6,125)
125           FORMAT('  ** ESTIMATED CORRECTIONS LARGER THAN 5*TOLAI ',
     1               'IN AIRPRP')
              IF (KSTEP.EQ.1) THEN
                WRITE(6, 130)
...
C
C
C  IF TOLAI .LT. 1, PREDICT NEXT STEP SIZE FROM LARGEST CORRECTION
C
        IF (TOLAI.LT.1.D0) THEN
C
...
C  COFF AND CDIAG SHOULD BE APPROXIMATELY TOLAI, SO FROM EQ. (27):
C  DRNOW(AT N+1) = (12 TOLAI/KBAR(N+1)W(N+1)-TILDA')**(1/3)
C  WHICH IS APPROX = (12 TOLAI/KBAR(N)W(N)-TILDA')**(1/3)
C                  = ((12 COFF/KBAR W-TILDA') (TOLAI/COFF))**(1/3)
C                  = DRNOW(AT N) (TOLAI/COFF)**(1/3)
C  OR FROM EQ. (29):
C                   DRNOW = DRNOW (TOLAI/CDIAG)**(1/3)
C
C  SG AND JMH MODIFICATIONS:
C     THE ORIGINAL STEP SIZE ALGORITHM WAS
C         DRNOW = DRNOW * (TOLAI/CMAX) ** (1.D0 / POWR)
C     BUT IT IS MORE REASONABLE TO LIMIT INCREMENT/DECREMENT FOR STABILITY
          FACTOR=(TOLAI/CMAX)**(1.D0/POWR)
...
C
        ELSE
C
C  AND NEXT STEP SIZE IS GEOMETRICALLY RELATED TO LAST STEP SIZE
C
          IF (DRNOW.GT.0.D0) DRNOW = TOLAI * DRNOW
          IF (DRNOW.LT.0.D0) DRNOW = DRNOW / TOLAI
        ENDIF
C
...
C
        RLAST = RNEXT
        RNEXT = RNEXT + DRNOW
C       IF (REND.GT.RSTART .AND. RNEXT.LT.REND) GOTO 170
C       IF (REND.LT.RSTART .AND. RNEXT.GT.REND) GOTO 170
        IF ((REND-RSTART)*(REND-RNEXT).GT.0.D0) GOTO 170
...
C
C     IF (IPRINT.GE.20) THEN
C       WRITE(6,305) RSTART, REND, TOLAI, NSTEP
C       WRITE(6,310) SPCMN, SPCMX, POWR
C305    FORMAT('         RSTART =', F10.3, '  REND  =', 1PG11.3,
C    :         '    TOLAI =', 1PE8.1, '  NINTERVAL =', I8)
...
315     FORMAT(/'  AIRY.   DRMIN =', 0PF10.4, '   DRMAX =', 1PG11.3$)
      ...
      END
C================================================================= END OF AIRPRP
      DOUBLE PRECISION FUNCTION GEOSTP(RST,REND,DR,TOLHI,NSTEPS)
C  Copyright (C) 2018 J. M. Hutson & C. R. Le Sueur
C  Distributed under the GNU General Public License, version 3
      IMPLICIT NONE
C  THIS FUNCTION CALCULATES THE END POINT OF A GEOMETRIC PROGRESSION FROM RST
C  WITH INITIAL STEP SIZE DR AND RATIO TOLHI.
C  IF NSTEPS IS NON-ZERO ON ENTRY, THE PROGRESSION USES NSTEPS STEPS,
C  OTHERWISE IT CALCULATES THE LAST POINT BEFORE REND IN THE GEOMETRIC PROGRESSION

      DOUBLE PRECISION, INTENT(IN)::RST,REND,DR,TOLHI
      INTEGER, INTENT(INOUT)::NSTEPS

      IF (NSTEPS.EQ.0) THEN
        NSTEPS=INT(LOG((REND-RST)/ABS(DR)*(TOLHI-1.D0)+1.D0)/LOG(TOLHI))
      ENDIF
      GEOSTP=RST+ABS(DR)*(TOLHI**NSTEPS-1.D0)/(TOLHI-1.D0)

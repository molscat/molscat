...
C  SET UP POWRX
      IF (POWRL.EQ.UNSET .AND. POWRS.EQ.UNSET) THEN
        POWRL=POWRX
        POWRS=1.D0
      ELSEIF (POWRL.EQ.UNSET) THEN
        POWRL=POWRS
      ELSEIF (POWRS.EQ.UNSET) THEN
        POWRS=1.D0
      ENDIF
      IF (IPROPL.NE.3 .AND. IPROPL.NE.9) POWRL=1.D0
      IF (IPROPS.NE.3 .AND. IPROPS.NE.9) POWRS=1.D0
      IF (NSEG.EQ.2) THEN
        IF (ABS(POWRL).LT.1D-8) THEN
          WRITE(6,*) ' *** THIS VALUE FOR POWRL IS INVALID: ',POWRL
          STOP
        ENDIF
      ENDIF
      IF (ABS(POWRS).LT.1D-8) THEN
        WRITE(6,*) ' *** THIS VALUE FOR POWRS IS INVALID: ',POWRS
        STOP
      ENDIF
